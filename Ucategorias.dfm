object frmcategorias: Tfrmcategorias
  Left = 329
  Top = 208
  Width = 525
  Height = 395
  Hint = 'Pantalla ABC de Categorias'
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'ABC de Categorias'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  ShowHint = True
  Visible = True
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnKeyUp = FormKeyUp
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lblclave: TRzLabel
    Left = 5
    Top = 13
    Width = 37
    Height = 13
    Caption = 'Clave:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel1: TRzLabel
    Left = 5
    Top = 49
    Width = 48
    Height = 13
    Caption = 'Nombre:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel2: TRzLabel
    Left = 5
    Top = 198
    Width = 72
    Height = 13
    Caption = 'Descripci'#243'n:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object lblgiro: TRzLabel
    Left = 234
    Top = 92
    Width = 28
    Height = 13
    Caption = 'Giro:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel3: TRzLabel
    Left = 5
    Top = 93
    Width = 73
    Height = 13
    Caption = 'Costo Anual:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel4: TRzLabel
    Left = 114
    Top = 93
    Width = 88
    Height = 13
    Caption = 'Costo Mensual:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object lbufh: TRzLabel
    Left = 13
    Top = 296
    Width = 13
    Height = 13
    Caption = '...'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMoneyGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel5: TRzLabel
    Left = 5
    Top = 140
    Width = 34
    Height = 13
    Caption = 'Color:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel6: TRzLabel
    Left = 338
    Top = 140
    Width = 90
    Height = 13
    Caption = 'Tipo Monitoreo:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object btnalta: TRzBitBtn
    Left = 8
    Top = 321
    Width = 88
    Height = 30
    Hint = 'Dar de Alta una Categoria'
    ModalResult = 1
    Caption = '&Alta'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    OnClick = btnaltaClick
    Glyph.Data = {
      
      FFFFFF777777777FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ImageIndex = 5
    NumGlyphs = 2
  end
  object btnborrar: TRzBitBtn
    Left = 97
    Top = 321
    Width = 94
    Height = 30
    Hint = 'Elimina la Categoria actual'
    ModalResult = 1
    Caption = '&Borrar'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 5
    OnClick = btnborrarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8091717E8E8E8E8E8E8E8E8E8E8E8E8E856ACAC}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btnmodificar: TRzBitBtn
    Left = 191
    Top = 321
    Width = 97
    Height = 30
    Hint = 'Editar los datos de la Categoria Actual'
    ModalResult = 1
    Caption = '&Editar'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 4
    OnClick = btnmodificarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btncancelar: TRzBitBtn
    Left = 289
    Top = 321
    Width = 97
    Height = 30
    Hint = 'Cerrar esta Pantalla'
    Caption = '&Cerrar'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    TabOrder = 6
    OnClick = btncancelarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object ednombre: TRzDBEdit
    Left = 5
    Top = 64
    Width = 376
    Height = 21
    Hint = 'Nombre de la Categoria'
    DataSource = DScategorias
    DataField = 'nombre'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FocusColor = clSkyBlue
    FrameColor = clActiveCaption
    FrameHotColor = clBtnFace
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 1
    OnKeyPress = ednombreKeyPress
  end
  object navegador: TRzDBNavigator
    Left = 140
    Top = 10
    Width = 244
    Height = 21
    DataSource = DScategorias
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    Hints.Strings = (
      'Primera Categoria'
      'Anterior Categoria'
      'Siguiente Categoria'
      'Ultima Categoria')
    TabOrder = 7
    OnClick = navegadorClick
    Color = clWhite
  end
  object tbherramientas: TToolBar
    Left = 312
    Top = 184
    Width = 178
    Height = 29
    Hint = 'Barra de Herramientas'
    Align = alNone
    AutoSize = True
    ButtonHeight = 23
    ButtonWidth = 29
    Caption = 'tbherramientas'
    EdgeBorders = [ebLeft, ebTop, ebRight, ebBottom]
    Images = DMAccesos.iledicion
    TabOrder = 8
    object btncopy: TToolButton
      Left = 0
      Top = 2
      Action = frmprincipal.EditCopy1
    end
    object btncut: TToolButton
      Left = 29
      Top = 2
      Action = frmprincipal.EditCut1
    end
    object btnpaste: TToolButton
      Left = 58
      Top = 2
      Action = frmprincipal.EditPaste1
    end
    object btndelete: TToolButton
      Left = 87
      Top = 2
      Action = frmprincipal.EditDelete1
    end
    object btnundo: TToolButton
      Left = 116
      Top = 2
      Action = frmprincipal.EditUndo1
    end
    object btnfuentes: TToolButton
      Left = 145
      Top = 2
      Hint = 'Fuentes'
      Caption = '&Fuentes'
      Enabled = False
      ImageIndex = 19
      OnClick = btnfuentesClick
    end
  end
  object redescripcion: TRzDBRichEdit
    Left = 5
    Top = 215
    Width = 484
    Height = 74
    Hint = 'Descripci'#243'n de la categoria'
    DataField = 'descripcion'
    DataSource = DScategorias
    Enabled = False
    ScrollBars = ssVertical
    TabOrder = 2
    OnEnter = redescripcionEnter
    OnExit = redescripcionExit
    OnKeyPress = redescripcionKeyPress
    DisabledColor = clWhite
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameVisible = True
  end
  object edclave: TRzButtonEdit
    Left = 50
    Top = 8
    Width = 86
    Height = 22
    Hint = 'Buscar en el catalogo'
    DisabledColor = clWhite
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotColor = clFuchsia
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 0
    OnChange = edclaveChange
    OnKeyPress = edclaveKeyPress
    ButtonKind = bkFind
    ButtonShortCut = 119
    FlatButtons = True
    OnButtonClick = edclaveButtonClick
  end
  object cbogiro: TRzComboBox
    Left = 231
    Top = 110
    Width = 151
    Height = 21
    Hint = 'Nombre del Giro al que pertenece esta Categoria'
    Ctl3D = False
    Enabled = False
    FrameController = DMAccesos.frmctr
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 9
    OnKeyPress = cbogiroKeyPress
    OnNotInList = cbogiroNotInList
  end
  object edCostoAnual: TRzDBEdit
    Left = 5
    Top = 111
    Width = 107
    Height = 21
    Hint = 'Costo Anual con IVA'
    DataSource = DScategorias
    DataField = 'CostoAnual'
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FrameController = DMAccesos.frmctr
    ParentFont = False
    TabOrder = 10
    OnExit = edCostoAnualExit
    OnKeyPress = edCostoAnualKeyPress
  end
  object edCostoMensual: TRzDBEdit
    Left = 119
    Top = 111
    Width = 108
    Height = 21
    Hint = 'Costo Mensual con IVA'
    DataSource = DScategorias
    DataField = 'CostoMensual'
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FrameController = DMAccesos.frmctr
    ParentFont = False
    TabOrder = 11
    OnExit = edCostoMensualExit
    OnKeyPress = edCostoMensualKeyPress
  end
  object edcolor: TRzColorEdit
    Left = 5
    Top = 160
    Width = 121
    Height = 21
    CustomColor = clBlue
    ColorDlgOptions = [cdFullOpen, cdAnyColor]
    DefaultColor = clOlive
    ShowSystemColors = True
    DisabledColor = clWhite
    Enabled = False
    FlatButtons = True
    FrameHotTrack = True
    FrameHotStyle = fsNone
    FrameVisible = True
    TabOrder = 12
  end
  object chkservcob: TRzDBCheckBox
    Left = 248
    Top = 152
    Width = 81
    Height = 25
    DataField = 'serviciocobrable'
    DataSource = DScategorias
    ValueChecked = '1'
    ValueUnchecked = '0'
    Caption = 'Servicio Cobrable'
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    TabOrder = 13
  end
  object cbtipo: TRzComboBox
    Left = 335
    Top = 158
    Width = 151
    Height = 21
    Ctl3D = False
    Enabled = False
    FrameController = DMAccesos.frmctr
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 14
    OnKeyPress = cbogiroKeyPress
    OnNotInList = cbogiroNotInList
  end
  object edcolor1: TRzDBEdit
    Left = 133
    Top = 159
    Width = 107
    Height = 21
    Hint = 'Costo Anual con IVA'
    DataSource = DScategorias
    DataField = 'color'
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotColor = clFuchsia
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 15
    OnKeyPress = edcolor1KeyPress
  end
  object DScategorias: TDataSource
    DataSet = tblcategorias
    Left = 296
    Top = 32
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    SQL.Strings = (
      'select telefono,tipo_tel from telefonos where idtel=:tel')
    Options.FullRefresh = True
    FetchAll = True
    Left = 88
    Top = 32
    ParamData = <
      item
        DataType = ftInteger
        Name = 'tel'
      end>
  end
  object tblcategorias: TMyTable
    TableName = 'categorias'
    Connection = DMAccesos.conexion
    AfterOpen = tblcategoriasAfterOpen
    AfterClose = tblcategoriasAfterClose
    BeforePost = tblcategoriasBeforePost
    FetchAll = True
    Left = 232
    Top = 32
    object tblcategoriasidcategoria: TIntegerField
      FieldName = 'idcategoria'
    end
    object tblcategoriasnombre: TStringField
      FieldName = 'nombre'
      Size = 30
    end
    object tblcategoriasdescripcion: TMemoField
      FieldName = 'descripcion'
      BlobType = ftMemo
    end
    object tblcategoriasidgiro: TIntegerField
      FieldName = 'idgiro'
    end
    object tblcategoriasfecha: TStringField
      FieldName = 'fecha'
      Size = 15
    end
    object tblcategoriashora: TStringField
      FieldName = 'hora'
      Size = 15
    end
    object tblcategoriasusuario: TStringField
      FieldName = 'usuario'
      Size = 30
    end
    object tblcategoriasCostoAnual: TFloatField
      FieldName = 'CostoAnual'
    end
    object tblcategoriasCostoMensual: TFloatField
      FieldName = 'CostoMensual'
    end
    object tblcategoriasidtipomonitoreo: TSmallintField
      FieldName = 'idtipomonitoreo'
    end
    object tblcategoriasserviciocobrable: TSmallintField
      FieldName = 'serviciocobrable'
    end
    object tblcategoriascolor: TStringField
      FieldName = 'color'
      Size = 15
    end
  end
  object fuentes: TFontDialog
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Left = 160
    Top = 32
  end
end
