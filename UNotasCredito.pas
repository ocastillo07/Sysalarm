...
var
  frmNotasCredito: TfrmNotasCredito;
...
procedure tfrmNotasCredito.LimpiaTotales;
begin
    edTotMat.Text:='0';
    edDescuento.Text:='0';
    edIva.Text:='0';
...
function TfrmNotasCredito.TraeNc(idnc : integer) : integer;
begin
chtecla := #13;
 //trae los datos de la Nota de Credito si existe
 querys('select * from NotasCredito where serie = "NC" and idnc='+edNumNc.Text,0,SqlNotasCred);
...
function TfrmNotasCredito.ValidaExistencias(CodCve:string;cantidad:integer;piezas:boolean):integer;
var
...
procedure TfrmNotasCredito.InsertaArticulosPaquete(cod_paquete:string;cantidad:integer);
...
procedure TfrmNotasCredito.edcvecteButtonClick(Sender: TObject);
var r : integer;
begin
...
procedure TfrmNotasCredito.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
action := cafree;
end;

procedure TfrmNotasCredito.edfacturaButtonClick(Sender: TObject);
...
if frmbusquedas.resultado <> '' then
 begin
  ...
  if querys('select pc.movimiento from PagosClientes pc where pc.idfactura='+quotedstr(edfactura.Text)+' and pc.movimiento in(2,8)',0,sqlgeneral) >0 then
     begin
     application.MessageBox('Esa tiene un pago, realice una Nota de Credito o cancele el cobro.','Pregunta',mb_iconinformation);
     sqlrejilla.close;
     exit;
     end;
...
procedure TfrmNotasCredito.ModificaArticulo;
begin
if btnalta.Caption = '&Guardar' then
  begin
  if (dsrejilla.DataSet.RecordCount > 0) and (dsrejilla.DataSet.FieldByName('paquete').AsString = '') then
    ...
    querys('delete from DetNotasCredito where id='+dsrejilla.DataSet.fieldbyname('id').AsString+' '+
           'and (cod_cve='+quotedstr(dsrejilla.DataSet.FieldValues['cod_cve'])+' or paquete='+quotedstr(dsrejilla.DataSet.FieldValues['cod_cve'])+') ',1,SqlGeneral);
    //refresca la rejilla de datos
    sqlrejilla.Close;
    sqlrejilla.ParamByName('maq').Value := strtoint(frmprincipal.idmaq);
...
procedure TfrmNotasCredito.MetodoAlta;
Var sqlarticulos : TMyquery;
begin
sqlarticulos := tmyquery.Create(self);
sqlarticulos.Connection := dmaccesos.conexion;
...
procedure TfrmNotasCredito.edcvecteChange(Sender: TObject);
begin
...
procedure tfrmNotasCredito.limpia;
begin
edventa.Clear;
ednumnc.clear;
edcvecte.Clear;
...
procedure TfrmNotasCredito.btnaltaClick(Sender: TObject);
//Var temp : string;
begin
if btnalta.Caption='&Alta' then
 ...
 begin
  ...
  querys('insert into NotasCredito(idnc,idcliente,idvendedor,idfactura,FechaNc,subtotal,impuesto,piva,anticipo,total,pedimento,'+
         'aduana,FechaAduana,FechaPago,observaciones,TipoCambio,saldo,usuario,fecha,hora) values ('+edNumNc.Text+','+edcvecte.Text+','+edcvevend.Text+','+quotedstr(edfactura.Text) +
...
procedure TfrmNotasCredito.edcvecteKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmNotasCredito.edcvecteExit(Sender: TObject);
begin
chtecla := #13;
if edcvecte.Text <> '' then
 edcvecte.OnKeyPress(self,chtecla);
...
procedure TfrmNotasCredito.FormShow(Sender: TObject);
begin
semiauto := 0;
...
function TfrmNotasCredito.ValidaProducto(CodCve:string):integer;
 ...
 begin
  if (dmreportes.sqlgeneral.fieldbyname('servicio').Asinteger = 0) and (dmreportes.sqlgeneral.fieldbyname('PrecioCapturable').Asinteger = 0) then //si se cumple significa que es un articulo
   ...
   begin
    ...
    querys('SELECT sum(((DetNotasCredito.cant * articulos.piezasxunidad) + DetNotasCredito.piezas)) as total FROM DetNotasCredito INNER JOIN articulos ON (DetNotasCredito.cod_cve = articulos.cod_cve) WHERE ((DetNotasCredito.idnc = '+quotedstr(edNumNc.Text)+') or (DetNotasCredito.idmaq = '+frmprincipal.idmaq+') or (DetNotasCredito.idpedido='+edventa.text+')) and DetNotasCredito.cod_cve='+quotedstr(CodCve),0,sqlgeneral);
...
procedure TfrmNotasCredito.edcodigoButtonClick(Sender: TObject);
begin
...
procedure TfrmNotasCredito.InsertaArticulo(cod_cve:string;cantidad:integer);
...
procedure TfrmNotasCredito.btnagregarClick(Sender: TObject);
var valor : integer;
begin
...
procedure TfrmNotasCredito.edCantidadKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmNotasCredito.LimpiaDatArt;
begin
     //limpia los campos
     btnagregar.Enabled:=false;
     edcantidad.Text:='';
...
procedure TfrmNotasCredito.FormCloseQuery(Sender: TObject;
...
procedure TfrmNotasCredito.edfacturaKeyPress(Sender: TObject;
  var Key: Char);
begin
       ...
       if SqlGeneral.RecordCount = 0 then
        ...
        begin
          if application.MessageBox('El numero de factura tecleado es del mes en curso, Desea Continuar?','Aviso',mb_iconquestion+mb_yesno) = idyes then
           begin
            //trae el numero de pedido
            querys('select pedidos.idpedido,pedidos.TipoCambio,pedidos.idvendedor '+
                   'from RemiFact inner join pedidos on pedidos.idpedido = RemiFact.idpedido '+
                   'where RemiFact.idfactura='+quotedstr(edfactura.Text),0,sqlgeneral);
            idpedido := sqlgeneral.fieldbyname('idpedido').AsString;
            edcvevend.Text := sqlgeneral.fieldbyname('idvendedor').AsString;
            edtcp.Text := sqlgeneral.fieldbyname('TipoCambio').AsString;
...
procedure TfrmNotasCredito.btncancelarClick(Sender: TObject);
begin
...
procedure TfrmNotasCredito.edCantidadChange(Sender: TObject);
begin
...
procedure TfrmNotasCredito.CalculaTotales;
...
procedure TfrmNotasCredito.EdPrecioKeyPress(Sender: TObject;
  var Key: Char);
begin
  key := ValidaNumeros(key,false);
...
procedure TfrmNotasCredito.EdPrecioExit(Sender: TObject);
begin
...
procedure TfrmNotasCredito.edcodigoKeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#27) then
 begin
...
procedure TfrmNotasCredito.edcodigoChange(Sender: TObject);
begin
if edcodigo.Text = '' then
LimpiaDatArt;
end;

procedure TfrmNotasCredito.edAnticipoKeyPress(Sender: TObject;
  var Key: Char);
begin
key:=ValidaNumeros(key,false);
end;

procedure TfrmNotasCredito.edAnticipoExit(Sender: TObject);
begin
...
procedure TfrmNotasCredito.btnfuentesClick(Sender: TObject);
begin
CambiaFont(mmobservaciones);
end;

procedure TfrmNotasCredito.edNumNCButtonClick(Sender: TObject);
begin
      frmbusquedas.query:='SELECT * FROM NotasCredito where serie = "NC"';
...
procedure TfrmNotasCredito.mmObservacionesChange(Sender: TObject);
begin
...
procedure TfrmNotasCredito.RejillaDBTableView1DblClick(Sender: TObject);
begin
ModificaArticulo;
CalculaTotales;
end;

procedure TfrmNotasCredito.edcvevendExit(Sender: TObject);
begin
//edcvevend.OnKeyPress(self,chtecla);
end;

procedure TfrmNotasCredito.edNumNCKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmNotasCredito.edNumPedKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmNotasCredito.edAduanaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmNotasCredito.dtpFechaAduanaKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then
 begin
...
procedure TfrmNotasCredito.dtpFecPagAntKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then
 begin
...
procedure TfrmNotasCredito.RejillaDBTableView1KeyUp(Sender: TObject;
...
procedure TfrmNotasCredito.btnimprimirClick(Sender: TObject);
...
{querys('SELECT '+quotedstr(edfactura.Text)+' as Factura,'+quotedstr(dec)+' as NumLet,dnc.idnc,dnc.idpedido,dnc.id,dnc.cod_cve, '+
       ...
       'WHERE c.idcliente='+edcvecte.Text+' and nc.idnc ='+edNumNc.text,0,dmreportes.sqlgeneral);   }
...
procedure TfrmNotasCredito.edNumNCChange(Sender: TObject);
begin
if edNumNc.text = '' then
 limpia;
end;

procedure TfrmNotasCredito.edTCPExit(Sender: TObject);
begin
if edTCP.Text = '' then
 begin
...
procedure TfrmNotasCredito.btnmodificarClick(Sender: TObject);
begin
...
procedure TfrmNotasCredito.actualiza;
begin
...
if querys('select dnc.cod_cve, dnc.cant from DetNotasCredito dnc left join tmpNC'+frmprincipal.idmaq+' t on '+
          'dnc.cod_cve = t.cod_cve where t.cod_cve is null  and dnc.idnc ='+ednumnc.text, 0, sqlgeneral) > 0 then
  begin
...
if querys('select t.* from  tmpNC'+frmprincipal.idmaq+' t where t.cod_cve not in(select cod_cve from DetNotasCredito where idnc = '+ednumnc.text+')', 0, sqlgeneral) > 0 then
  begin
  sqlgeneral.First;
  while not sqlgeneral.Eof = true do
    begin
    ...
    if (sqlinserta.FieldByName('paquete').AsInteger=0)and(sqlinserta.FieldByName('servicio').AsInteger=0) then
      begin
      querys('insert into DetNotasCredito(idnc, cant,piezas,cod_cve,utilidad,costo,idmaq,idpedido,usuario,fecha,hora,tc) '+
             'values ('+ednumnc.text+','+sqlgeneral.FieldByName('cant').Asstring+','+floattostr(sqlinserta.fieldbyname('costo').AsFloat)+
...
function TfrmNotasCredito.ValidaTipoArticulo(CodCve:string):boolean;
begin
