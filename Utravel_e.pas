...
uses
  ...
  cxGridDBTableView, cxGrid, RzBckgnd, DateUtils;
...
type
  Tfrmtravel = class(TForm)
    ...
    procedure CboFiltraAreaClick(Sender: TObject);
    procedure CboFiltraAreaKeyPress(Sender: TObject; var Key: Char);
    procedure sqltravelAfterOpen(DataSet: TDataSet);
    procedure btnaltaClick(Sender: TObject);
    procedure btnmodificarClick(Sender: TObject);
    procedure TBBorrarClick(Sender: TObject);
    procedure btncancelarClick(Sender: TObject);
    procedure tsfichaClick(Sender: TObject);
...
querys('Select travel.idtravel,idservicio,travel.idcliente,travel.nombre AS Cliente,solicitante,telefono,ComentariosAuditor,idProblemaCausa,'+
       ' quejas.nombre as queja,quejas.idqueja,travel.idusraudito,travel.idusra,areas.nombre AS Depto,travel.FechaRecive,travel.HoraRecive,travel.descripcion,magnitudes.nombre AS Magnitud,quejas.nombre,travel.idfuente,travel.HoraAudito '+
       ' ,travel.idasunto,if(travel.resuelto=1,''Si'',''No'') AS Resuelto,if(travel.auditado=1,''Si'',''No'') AS Auditado,travel.idusrrecive,travel.iddptorecive,travel.idusraudito,travel.FechaAudito,travel.comunico,travel.ComunicoOtro,travel.idestatus, '+
       ' travel.accion From travel  left Join usuarios ON travel.idusra = usuarios.idusuario left Join areas ON travel.iddptoa = areas.idarea left Join magnitudes ON travel.idmagnitud = '+
       ' magnitudes.idmagnitud left Join quejas ON travel.idqueja = quejas.idqueja left Join asuntos ON travel.idasunto = asuntos.idasunto where idtravel='+id,0,sqlgral);
...
querys('select concat(ifnull(usuarios.nombre,''''),'' '',ifnull(usuarios.apaterno,''''),'' '',ifnull(usuarios.amaterno,'''')) AS Usuario from usuarios where idusuario='+edcve_usrrec.Text,0,sqlgeneral);
ednom_usrrec.Text := sqlgeneral.fieldbyname('usuario').AsString;
chtecla:=#13;
//edcve_usrrec.OnKeyPress(self,chtecla);
...
querys('select concat(ifnull(usuarios.nombre,''''),'' '',ifnull(usuarios.apaterno,''''),'' '',ifnull(usuarios.amaterno,'''')) AS Usuario from usuarios where idusuario='+sqlgral.fieldbyname('idusraudito').AsString,0,sqlgeneral);
...
querys('Select travel.idtravel,travel.idcliente,travel.nombre AS Cliente,'+
       'concat(ifnull(usuarios.nombre,''''),'' '',ifnull(usuarios.apaterno,''''),'' '',ifnull(usuarios.amaterno,'''')) AS Usuario,areas.nombre AS Depto,travel.FechaRecive,travel.HoraRecive,travel.descripcion,magnitudes.nombre AS Magnitud,quejas.nombre'+
       ' AS Queja,asuntos.nombre AS Asunto,if(travel.resuelto = 1,''Si'',''No'') as Resuelto,if(travel.auditado = 1,''Si'',''No'') as Auditado,idservicio From travel  left Join usuarios ON '+
       'travel.idusra = usuarios.idusuario left Join areas ON travel.iddptoa = areas.idarea left Join magnitudes ON travel.idmagnitud = magnitudes.idmagnitud left Join quejas ON travel.idqueja = quejas.idqueja left Join asuntos ON '+
       ' travel.idasunto = asuntos.idasunto where travel.resuelto=0 and travel.auditado=0 order by depto, resuelto',0,sqltravel);
end;
          ...
          if frmbusquedas.resultado <> '' then
            begin
                 edcvecte.Text := frmbusquedas.resultado;
                    ...
                    //         ' from clientes inner join telefonos on clientes.idtel = telefonos.idtel where clientes.idcliente='+edcvecte.Text+' and (telefonos.tipo_tel=''CASA'' or telefonos.tipo_tel=''ALARMA'') limit 1',0,sqlgeneral);
          ...
          if frmbusquedas.resultado <> '' then
            begin
                 edcvecte.Text := frmbusquedas.resultado;
                    ...
                    //         ' from clientes inner join telefonos on clientes.idtel = telefonos.idtel where clientes.idcliente='+edcvecte.Text+' and (telefonos.tipo_tel=''CASA'' or telefonos.tipo_tel=''ALARMA'') limit 1',0,sqlgeneral);
                 //verifica que lo que se halla tecleado en cve_cte sea valido
                 querys('select idproveedor,'+ ClienteNombre('proveedores') +' as Nombre From proveedores where idprov='+edcvecte.Text,0,sqlgeneral);
                 ednomcte.Text := sqlgeneral.fieldbyname('Nombre').AsString;
                 cboareadestino.SetFocus;
            end;
end;
...
begin
querys('SELECT usuarios.idusuario,usuarios.email1,usuarios.email2 FROM usuarios INNER JOIN puestos ON (usuarios.idpuesto = puestos.idpuesto) WHERE  (usuarios.estatus = 1 and concat(ifnull(usuarios.nombre,'' ''),'' '',ifnull(usuarios.apaterno,''''),'' '','+
       'ifnull(usuarios.amaterno,'' '')) ='+quotedstr(cbonom_vend.Text)+')',0,SqlGeneral);
edcve_vend.Text:=SqlGeneral.fieldbyname('idusuario').AsString;
cbonom_vend.Tag :=SqlGeneral.fieldbyname('idusuario').AsInteger;
...
procedure Tfrmtravel.TBGrabarClick(Sender: TObject);
//var resuelto,auditado,emailusr,emailusr2,emailjefe,mailtravel:string;
begin{
if edcve_vend.Text = '' then
 begin
...
if edcvecte.Text = '' then
 begin
  application.MessageBox('No podra guardar este travel hasta que no especifique la cuenta!','Aviso',mb_iconinformation);
  exit;
 end;
...
querys('select email2,email1,concat(ifnull(nombre,''''),'' '',ifnull(apaterno,''''),'' '',ifnull(amaterno,'''')) as Nombre  from usuarios where idusuario='+edcve_usrrec.Text,0,sqlgeneral);
if (sqlgeneral.fieldbyname('email1').AsString = '') and (sqlgeneral.fieldbyname('email2').AsString = '') then
 begin
...
querys('select email2,email1,concat(ifnull(nombre,''''),'' '',ifnull(apaterno,''''),'' '',ifnull(amaterno,'''')) as Nombre  from usuarios where idusuario='+edcve_vend.Text,0,sqlgeneral);
if (sqlgeneral.fieldbyname('email1').AsString = '') and (sqlgeneral.fieldbyname('email2').AsString = '') then
 begin
  application.MessageBox(pchar('El Usuario con nombre '+sqlgeneral.FieldByName('nombre').Asstring+' no se le ha capturado email, favor de ir a la pantalla usuarios y capturelo'),'Aviso',mb_iconwarning);
  querys('Select valor from configuraciones where concepto = ''UserEmailInterno''', 0, sqlgeneral);
...
querys('select usuarios.email1 from usuarios inner join puestos on puestos.idpuesto = usuarios.idpuesto inner join areas on areas.idarea = usuarios.idarea where areas.nombre='+quotedstr(cboAreadestino.Text)+' and  puestos.nombre=''GERENTE''',0,sqlgeneral);
sqlgeneral.First;
...
querys('select idusra from travel where idtravel='+ednumtravel.Text,0,sqlgeneral);
if (btnalta.Caption = '&Guardar')  then //esta en alta
begin
 if application.MessageBox('Todos los datos capturados son correctos ya que algunos de estos no podran ser modificados despues?','Pregunta',mb_iconquestion+mb_yesno)=idno then
  exit
 else
  begin
  ...
  // querys('select email1,email2 from usuarios where audittravel = 1',0,sqlgeneral);
...
if send_email(emailusr,emailusr2,emailjefe+','+emailusr+','+mailtravel,'Se ha resuelto un traveler. cuenta #'+edcvecte.Text+' '+ednomcte.Text,frmprincipal.mmdatos,body,1) = 1 then
   application.MessageBox('No se pudo enviar email a la persona encargada de este Traveler','Aviso',mb_iconinformation);
end;

//Cambio de usuario al que se dirige
if (frmprincipal.PuestoUsuario='GERENTE') and (frmprincipal.AreaUsuario = cboareadestino.Text) and (tbeditar.Tag = 1) then
  begin
...
tbadd.Tag := 0;
tbeditar.Tag := 0;
chkfiltroresueltos.Enabled := true;
chkfiltroauditados.Enabled := true;
...
querys('Select travel.idtravel,clientes.idcliente,if(clientes.rsocial='' ''OR clientes.rsocial Is Null,concat(ifnull(clientes.nombre,''''),'' '',ifnull(clientes.apaterno,''''),'' '',ifnull(clientes.amaterno,'''')),clientes.rsocial) AS Cliente,'+
       'concat(ifnull(usuarios.nombre,''''),'' '',ifnull(usuarios.apaterno,''''),'' '',ifnull(usuarios.amaterno,'''')) AS Usuario,areas.nombre AS Depto,travel.FechaRecive,travel.HoraRecive,travel.descripcion,magnitudes.nombre AS Magnitud,quejas.nombre'+
       ' AS Queja,asuntos.nombre AS Asunto,if(travel.resuelto = 1,''Si'',''No'') as Resuelto,if(travel.auditado = 1,''Si'',''No'') as Auditado,idservicio From travel left Join clientes ON travel.idcliente = clientes.idcliente left Join usuarios ON '+
       'travel.idusra = usuarios.idusuario left Join areas ON travel.iddptoa = areas.idarea left Join magnitudes ON travel.idmagnitud = magnitudes.idmagnitud left Join quejas ON travel.idqueja = quejas.idqueja left Join asuntos ON '+
       ' travel.idasunto = asuntos.idasunto order by travel.idtravel',0,sqltravel);
end;

procedure Tfrmtravel.CboFiltraAreaClick(Sender: TObject);
//var cadena:string;
begin
{cadena := 'Select t.idtravel,c.idcliente,'+ClienteNombre('c')+' AS Cliente, '+UsuarioNombre('u')+' AS Usuario, a.nombre AS Depto, '+
          't.FechaRecive, t.HoraRecive, t.descripcion, m.nombre AS Magnitud, q.nombre AS Queja, asu.nombre AS Asunto, '+
          'if(t.resuelto = 1,"Si","No") as Resuelto, if(t.auditado = 1, "Si", "No") as Auditado, idservicio From travel as t left '+
          'Join clientes as c ON t.idcliente = c.idcliente left Join usuarios as u ON t.idusra = u.idusuario left Join areas as a '+
          'ON t.iddptoa = a.idarea left Join magnitudes as m ON t.idmagnitud = m.idmagnitud left Join quejas as q ON t.idqueja = '+
          'q.idqueja left Join asuntos as asu ON t.idasunto = asu.idasunto ';
if chkfiltroTodos.Checked then
  begin
  if CboFiltraArea.Text = 'TODOS' then
    querys(cadena, 0, sqltravel)
  else
    querys(cadena+'where a.nombre='+quotedstr(CboFiltraArea.Text), 0, sqltravel);
  end
else
  begin
  if CboFiltraArea.Text <> 'TODOS' then
    querys( cadena+'where a.nombre='+quotedstr(CboFiltraArea.Text)+' and t.resuelto='+inttostr(chkfiltroresueltos.tag)+' and t.auditado='+inttostr(chkfiltroauditados.tag)+' order by t.idtravel',0,sqltravel)
  else
    querys(cadena+ 'where t.resuelto='+inttostr(chkfiltroresueltos.tag)+' and t.auditado='+inttostr(chkfiltroauditados.tag)+' order by t.idtravel',0,sqltravel)
  end;
        }
end;
...
procedure Tfrmtravel.sqltravelAfterOpen(DataSet: TDataSet);
begin
//lbltravelers.Caption := 'Travelers: '+inttostr(dataset.RecordCount);
end;
...
if btnAlta.Caption = '&Alta' then
  ...
  if btnalta.Caption = '&Guardar' then
    ...
    if application.MessageBox('Todos los datos capturados son correctos ya que algunos de estos no podran ser modificados despues?','Pregunta',mb_iconquestion+mb_yesno)=idno then
      ...
      querys('insert into travel(idtravel,idcliente,idasunto,idfuente,idqueja,iddptorecive,iddptoa,idusrrecive,idusra,idmagnitud,FechaRecive,HoraRecive,resuelto,auditado,idusraudito,FechaAudito,comunico,ComunicoOtro,descripcion,accion,'+
      'solicitante,nombre,telefono,ComentariosAuditor,idProblemaCausa) values ('+edNumTravel.Text+','+edcvecte.Text+','+inttostr(cboasunto.Tag)+','+inttostr(cbofuente.Tag)+','+inttostr(cboquejas.Tag)+','+inttostr(cboAreaRecive.Tag)+','+inttostr(cboAreaDestino.Tag)+','+
...
if btnmodificar.Caption = '&Editar' then
  ...
  querys('select audittravel,puestos.nombre as puesto,areas.nombre as area,usuarios.idusuario from usuarios inner join puestos on usuarios.idpuesto = puestos.idpuesto inner join areas on areas.idarea = usuarios.idarea where usuarios.idusuario='+inttostr(frmprincipal.ucprincipal.CurrentUser.UserID),0,sqlgeneral);
  if (sqlgeneral.fieldbyname('idusuario').asstring <> edcve_vend.Text)and(sqlgeneral.fieldbyname('audittravel').asstring<>'1') then
    begin
  ...
  if sqlgeneral.fieldbyname('audittravel').asstring='1' then
    begin
    chkauditado.Enabled := true;
    cboaudito.Enabled := true;
...
procedure Tfrmtravel.tsfichaClick(Sender: TObject);
begin
 { if validaguardar = false then
    exit;
...
var cadena:string;
begin
cadena := 'Select t.idtravel,c.idcliente,'+ClienteNombre('c')+' AS Cliente, '+UsuarioNombre('u')+' AS Usuario, '+
          ...
          'idservicio From travel as t left Join clientes as c ON t.idcliente = c.idcliente left Join usuarios as u ON '+
...
case cboAltaTravel.ItemIndex of
     0: lblBusqueda.Caption := 'Empleado';
