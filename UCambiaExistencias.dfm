object frmCambiaExistencias: TfrmCambiaExistencias
  Left = 469
  Top = 260
  Width = 512
  Height = 157
  AutoSize = True
  Caption = 'Cambia Existencias de Articulos'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object RzLabel41: TRzLabel
    Left = 0
    Top = 0
    Width = 37
    Height = 13
    Caption = 'Clave:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel4: TRzLabel
    Left = 4
    Top = 33
    Width = 48
    Height = 13
    Caption = 'Nombre:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object edPiezasExistencia: TRzLabel
    Left = 212
    Top = 83
    Width = 69
    Height = 13
    Caption = 'Existencias:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel1: TRzLabel
    Left = 8
    Top = 83
    Width = 81
    Height = 13
    Caption = 'En Apartados:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel2: TRzLabel
    Left = 104
    Top = 83
    Width = 82
    Height = 13
    Caption = 'En Instalacion'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel3: TRzLabel
    Left = 400
    Top = 32
    Width = 100
    Height = 13
    Caption = 'Existencia Fisica:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object edCveArt: TRzButtonEdit
    Left = 47
    Top = 0
    Width = 126
    Height = 21
    Hint = 'Buscar en el catalogo'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotColor = clFuchsia
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 0
    OnChange = edCveArtChange
    OnKeyPress = edCveArtKeyPress
    ButtonKind = bkFind
    ButtonShortCut = 119
    FlatButtons = True
    OnButtonClick = edCveArtButtonClick
  end
  object ednombre: TRzDBEdit
    Left = 6
    Top = 48
    Width = 379
    Height = 21
    Hint = 'Nombre del Articulo'
    DataSource = dsarticulos
    DataField = 'nombre'
    ReadOnly = True
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotTrack = True
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
  end
  object edExistencias: TRzDBEdit
    Left = 209
    Top = 102
    Width = 76
    Height = 21
    Hint = 'Codigo de Barras del Articulo'
    DataSource = dsarticulos
    DataField = 'existencia'
    ReadOnly = True
    Alignment = taRightJustify
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotTrack = True
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
  end
  object btnactualizar: TRzBitBtn
    Left = 316
    Top = 88
    Width = 89
    Height = 33
    Caption = '&Editar'
    HotTrack = True
    TabOrder = 3
    OnClick = btnactualizarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object navegador: TRzDBNavigator
    Left = 188
    Top = 1
    Width = 316
    Height = 25
    DataSource = dsarticulos
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    TabOrder = 4
    OnClick = navegadorClick
    Color = clWhite
  end
  object btnsalir: TRzBitBtn
    Left = 414
    Top = 88
    Width = 83
    Height = 33
    Caption = 'Salir'
    HotTrack = True
    TabOrder = 5
    OnClick = btnsalirClick
    Glyph.Data = {
      
      57E8E8E8E8E8E8E8E8E8E8E8E8AC818181E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object edapartado: TRzDBEdit
    Left = 8
    Top = 102
    Width = 76
    Height = 21
    DataSource = dsarticulos
    DataField = 'apartado'
    Alignment = taRightJustify
    DisabledColor = clWhite
    Enabled = False
    FrameHotTrack = True
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 6
  end
  object edinstalacion: TRzDBEdit
    Left = 104
    Top = 102
    Width = 76
    Height = 21
    DataSource = dsarticulos
    DataField = 'instalacion'
    Alignment = taRightJustify
    DisabledColor = clWhite
    Enabled = False
    FrameHotTrack = True
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 7
  end
  object edreal: TRzEdit
    Left = 400
    Top = 48
    Width = 97
    Height = 21
    Alignment = taRightJustify
    DisabledColor = clWhite
    Enabled = False
    FrameHotTrack = True
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 8
    OnExit = edrealExit
    OnKeyPress = edrealKeyPress
  end
  object dsarticulos: TDataSource
    DataSet = tblarticulos
    Left = 257
    Top = 65533
  end
  object tblarticulos: TMyTable
    TableName = 'articulos'
    Connection = DMAccesos.conexion
    AfterPost = tblarticulosAfterPost
    AfterScroll = tblarticulosAfterScroll
    FilterSQL = 'status=1 and inventariable=1'
    FetchAll = True
    Left = 409
    Top = 65533
    object tblarticuloscod_cve: TStringField
      FieldName = 'cod_cve'
      Origin = 'articulos.cod_cve'
      Size = 30
    end
    object tblarticuloscod_barras: TStringField
      FieldName = 'cod_barras'
      Origin = 'articulos.cod_barras'
      Size = 30
    end
    object tblarticulosnombre: TStringField
      FieldName = 'nombre'
      Origin = 'articulos.nombre'
      Size = 80
    end
    object tblarticulosexistencia: TIntegerField
      FieldName = 'existencia'
      Origin = 'articulos.existencia'
    end
    object tblarticulosapartado: TIntegerField
      FieldName = 'apartado'
      Origin = 'articulos.apartado'
    end
    object tblarticulosinstalacion: TIntegerField
      FieldName = 'instalacion'
      Origin = 'articulos.instalacion'
    end
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    FetchAll = True
    Left = 328
    Top = 1
  end
end
