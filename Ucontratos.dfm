object frmcontratos: Tfrmcontratos
  Left = 344
  Top = 331
  Width = 419
  Height = 228
  Hint = 'Pantalla de Tipos de Contratos'
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'ABC de Tipos de Contratos'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnClose = FormClose
  OnKeyUp = FormKeyUp
  PixelsPerInch = 96
  TextHeight = 13
  object lblclave: TRzLabel
    Left = 8
    Top = 13
    Width = 37
    Height = 13
    Caption = 'Clave:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel1: TRzLabel
    Left = 8
    Top = 41
    Width = 48
    Height = 13
    Caption = 'Nombre:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel13: TRzLabel
    Left = 8
    Top = 85
    Width = 47
    Height = 13
    Hint = 'Estatus del cliente actual'
    Caption = 'Estatus:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object lbufh: TRzLabel
    Left = 8
    Top = 129
    Width = 13
    Height = 13
    Caption = '...'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMoneyGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object btnalta: TRzBitBtn
    Left = 8
    Top = 154
    Width = 97
    Height = 30
    Hint = 'Dar de Alta a un tipo de contrato '
    ModalResult = 1
    Caption = '&Alta'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
    OnClick = btnaltaClick
    Glyph.Data = {
      
      FFFFFF777777777FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ImageIndex = 5
    NumGlyphs = 2
  end
  object btnborrar: TRzBitBtn
    Left = 104
    Top = 154
    Width = 97
    Height = 30
    Hint = 'Elimina el contrato actual'
    ModalResult = 1
    Caption = '&Borrar'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 5
    OnClick = btnborrarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8091717E8E8E8E8E8E8E8E8E8E8E8E8E856ACAC}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btnmodificar: TRzBitBtn
    Left = 200
    Top = 154
    Width = 97
    Height = 30
    Hint = 'Editar los datos del tipo contrato actual'
    ModalResult = 1
    Caption = '&Editar'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    OnClick = btnmodificarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btncancelar: TRzBitBtn
    Left = 296
    Top = 154
    Width = 97
    Height = 30
    Hint = 'Cerrar esta Pantalla'
    Caption = '&Cerrar'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 4
    OnClick = btncancelarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object navegador: TRzDBNavigator
    Left = 175
    Top = 11
    Width = 224
    Height = 28
    Hint = 'ABC de Tipos de contratos'
    DataSource = DScontratos
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    Hints.Strings = (
      'Primer Tipo de Contrato'
      'Anterior Tipo de Contrato'
      'Siguiente Tipo de Contrato'
      'Ultima Tipo de Contrato')
    ParentShowHint = False
    ShowHint = True
    TabOrder = 6
    OnClick = navegadorClick
    Color = clWhite
  end
  object edclave: TRzButtonEdit
    Left = 48
    Top = 13
    Width = 86
    Height = 22
    Hint = 'Buscar en el catalogo'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotColor = clFuchsia
    FrameSides = [sdBottom]
    FrameVisible = True
    MaxLength = 3
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    OnChange = edclaveChange
    OnKeyPress = edclaveKeyPress
    ButtonKind = bkFind
    ButtonShortCut = 119
    FlatButtons = True
    OnButtonClick = edclaveButtonClick
  end
  object ednombre: TRzDBEdit
    Left = 8
    Top = 56
    Width = 377
    Height = 21
    Hint = 'Nombre de la Fuente'
    DataSource = DScontratos
    DataField = 'nombre'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FocusColor = clSkyBlue
    FrameColor = clActiveCaption
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    OnKeyPress = ednombreKeyPress
  end
  object cboestatus: TRzComboBox
    Left = 8
    Top = 101
    Width = 145
    Height = 21
    Hint = 'Estatus del cliente actual'
    Ctl3D = False
    Enabled = False
    FrameController = DMAccesos.frmctr
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 7
    Items.Strings = (
      'INACTIVO'
      'ACTIVO'
      'POR REVISAR'
      'CORTESIA'
      'MOROSO'
      'CANCELADO'
      'LIBRE')
  end
  object chkvigencia: TRzDBCheckBox
    Left = 168
    Top = 102
    Width = 113
    Height = 19
    Hint = 'Para indicar en la pantalla de clientes cuando vence el contrato'
    DataField = 'TieneVigencia'
    DataSource = DScontratos
    ValueChecked = '1'
    ValueUnchecked = '0'
    Caption = 'Tiene Vigencia'
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    LightTextStyle = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 8
  end
  object DScontratos: TDataSource
    DataSet = tblcontratos
    Left = 320
    Top = 8
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    SQL.Strings = (
      '')
    Options.FullRefresh = True
    FetchAll = True
    Left = 184
    Top = 8
  end
  object tblcontratos: TMyTable
    TableName = 'contratos'
    Connection = DMAccesos.conexion
    BeforePost = tblcontratosBeforePost
    FetchAll = True
    Left = 240
    Top = 8
    object tblcontratosidcontrato: TIntegerField
      FieldName = 'idcontrato'
    end
    object tblcontratosTipoCont: TStringField
      FieldName = 'TipoCont'
      FixedChar = True
      Size = 3
    end
    object tblcontratosnombre: TStringField
      FieldName = 'nombre'
      Size = 30
    end
    object tblcontratosestatus: TSmallintField
      FieldName = 'estatus'
    end
    object tblcontratosabogado: TSmallintField
      FieldName = 'abogado'
    end
    object tblcontratosusuario: TStringField
      FieldName = 'usuario'
      Size = 30
    end
    object tblcontratosfecha: TStringField
      FieldName = 'fecha'
      Size = 15
    end
    object tblcontratoshora: TStringField
      FieldName = 'hora'
      Size = 15
    end
    object tblcontratoscancelado: TSmallintField
      FieldName = 'cancelado'
    end
    object tblcontratosremisiona: TIntegerField
      FieldName = 'remisiona'
    end
    object tblcontratosTieneVigencia: TIntegerField
      FieldName = 'TieneVigencia'
    end
  end
end
