...
implementation
uses UCBase, Uprincipal, Udm, Urecursos, UEmpresas;
{$R *.dfm}
...
//sync
lblconexion.Caption:= 'Servidor: '+DMAccesos.conexion.server;
lblbd.Caption      := 'Base de Datos: '+DMAccesos.conexion.Database;
...
begin
editusuario.Color:=clGradientActiveCaption;
end;
...
begin
editsenha.Color:=clGradientActiveCaption;
end;
...
procedure TLoginWindow.btnempresasClick(Sender: TObject);
var hija:Tfrmempresas;
begin
  hija:=tfrmempresas.Create(nil);
  hija.ShowModal;
  frmprincipal.OnActivate(self);
  self.OnShow(self);
end;
