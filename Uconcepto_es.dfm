object frmconcepto_es: Tfrmconcepto_es
  Left = 335
  Top = 183
  Width = 432
  Height = 217
  Caption = 'ABC Conceptos de E/S'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnKeyUp = FormKeyUp
  PixelsPerInch = 96
  TextHeight = 13
  object lblclave: TRzLabel
    Left = 10
    Top = 13
    Width = 37
    Height = 13
    Caption = 'Clave:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel1: TRzLabel
    Left = 10
    Top = 33
    Width = 48
    Height = 13
    Caption = 'Nombre:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object lbufh: TRzLabel
    Left = 128
    Top = 113
    Width = 13
    Height = 13
    Caption = '...'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMoneyGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object btnalta: TRzBitBtn
    Left = 11
    Top = 140
    Width = 96
    Height = 30
    Hint = 'Dar de Alta a un Concepto de Entrada / Salida'
    ModalResult = 1
    Caption = '&Alta'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    OnClick = btnaltaClick
    Glyph.Data = {
      
      FFFFFF777777777FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ImageIndex = 5
    NumGlyphs = 2
  end
  object btnborrar: TRzBitBtn
    Left = 110
    Top = 140
    Width = 94
    Height = 30
    Hint = 'Elimina el Concepto de Entrada / Salida actual'
    ModalResult = 1
    Caption = '&Borrar'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    OnClick = btnborrarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8091717E8E8E8E8E8E8E8E8E8E8E8E8E856ACAC}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btnmodificar: TRzBitBtn
    Left = 210
    Top = 140
    Width = 96
    Height = 30
    Hint = 'Editar los datos del Concepto de Entrada / Salida  Actual'
    ModalResult = 1
    Caption = '&Editar'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
    OnClick = btnmodificarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btncancelar: TRzBitBtn
    Left = 309
    Top = 140
    Width = 95
    Height = 30
    Hint = 'Cerrar esta Pantalla'
    Caption = '&Cerrar'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    TabOrder = 3
    OnClick = btncancelarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object ednombre: TRzDBEdit
    Left = 8
    Top = 56
    Width = 393
    Height = 21
    Hint = 'Nombre del Concepto de Entrada / Salida'
    DataSource = DSconceptos_es
    DataField = 'nombre'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FocusColor = clSkyBlue
    FrameColor = clActiveCaption
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 4
    OnKeyPress = ednombreKeyPress
  end
  object rgfinaliza: TRzDBRadioGroup
    Left = 9
    Top = 88
    Width = 112
    Height = 41
    DataField = 'movimiento'
    DataSource = DSconceptos_es
    Items.Strings = (
      'E'
      'S')
    Caption = 'Tipo Mov'
    Color = clWhite
    Columns = 2
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ItemHotTrack = True
    ParentFont = False
    TabOrder = 5
    OnEnter = rgfinalizaEnter
  end
  object navegador: TRzDBNavigator
    Left = 141
    Top = 11
    Width = 152
    Height = 32
    DataSource = DSconceptos_es
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    Hints.Strings = (
      'Primer Concepto de Entrada / Salida'
      'Anterior Concepto de Entrada / Salida'
      'Siguiente Concepto de Entrada / Salida'
      'Ultimo Concepto de Entrada / Salida')
    TabOrder = 6
    OnClick = navegadorClick
    Color = clWhite
  end
  object edclave: TRzButtonEdit
    Left = 50
    Top = 8
    Width = 86
    Height = 22
    Hint = 'Buscar en el catalogo'
    DisabledColor = clWhite
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotColor = clFuchsia
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 7
    OnChange = edclaveChange
    OnKeyPress = edclaveKeyPress
    ButtonKind = bkFind
    ButtonShortCut = 119
    FlatButtons = True
    OnButtonClick = edclaveButtonClick
  end
  object DSconceptos_es: TDataSource
    DataSet = tblconceptos_es
    Left = 200
    Top = 24
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    SQL.Strings = (
      'select telefono,tipo_tel from telefonos where idtel=:tel')
    Options.FullRefresh = True
    FetchAll = True
    Left = 96
    Top = 24
    ParamData = <
      item
        DataType = ftInteger
        Name = 'tel'
      end>
  end
  object tblconceptos_es: TMyTable
    TableName = 'ConceptoES'
    Connection = DMAccesos.conexion
    FetchAll = True
    Left = 152
    Top = 24
    object tblconceptos_esidconcepto_es: TIntegerField
      FieldName = 'idconcepto_es'
    end
    object tblconceptos_esnombre: TStringField
      FieldName = 'nombre'
      Size = 50
    end
    object tblconceptos_esmovimiento: TStringField
      FieldName = 'movimiento'
      FixedChar = True
      Size = 3
    end
    object tblconceptos_esusuario: TStringField
      FieldName = 'usuario'
      Size = 30
    end
    object tblconceptos_esfecha: TStringField
      FieldName = 'fecha'
      Size = 15
    end
    object tblconceptos_eshora: TStringField
      FieldName = 'hora'
      Size = 15
    end
  end
end
