   ...
   begin
         ...
         begin
            ...
            begin
               ...
               application.MessageBox('La actualización de datos por parte de la oportunidad fue realizada!','Información',mb_IconInformation);
            end;
...
frmprincipal.ucprincipal.Log('Edito la instalación '+tblinstalacionesidinstalacion.AsString,2);
...
btnmodificar.hint:='Editar los datos de la instalación';
edbuscar.SetFocus;
...
//if tblinstalaciones.Locate(cadena,edbuscar.Text,[]) then
  ...
  with dmaccesos do
    ...
    cadena:='SELECT i.instalador1 as i1,i.instalador2 as i2,i.idinstalacion as idi,p.idpedido as Venta,p.idPedidoPadre as PedidoPadre, '+
     ...
     if sqlgeneral.fieldbyname('Estatus').AsString = 'CANCELADA' then
        application.MessageBox('La venta esta cancelada, consulte con el encargo de Remisiones!','Atención',mb_IconInformation);
  ...
  begin
    ...
    application.MessageBox('No existe intente de nuevo','Información',mb_ok+mb_iconinformation);
...
if dmaccesos.sqlgeneral.RecordCount >0 then begin                                                                                                                    //p.estatus in (3,4) and
    ...
    frmbusquedas.query:='SELECT idinstalacion,(CASE  1 WHEN  i.EstatusInst=0 THEN ''PENDIENTE'' WHEN  i.EstatusInst=1 THEN ''INSTALANDO'' WHEN  i.EstatusInst=2 THEN ''SUPERVISION'' WHEN  i.EstatusInst=3 THEN ''SUPERV. C/PROB INST'' WHEN  EstatusInst=4 THEN ''UBICACION'' '+
             ...
             if ban3=0 then begin
                ...
                frmbusquedas.lbldesc.Caption:='Busqueda por Instalación:';
            end else begin
  ...
  application.MessageBox('No hay pedidos pendientes','Atención',mb_Iconinformation);
...
begin
     ...
     if btnmodificar.Caption = '&Guardar' then
        begin
            ...
            btnmodificar.Caption:='&Editar';
            btncancelar.Hint:='Cerrar esta Pantalla';
            btnmodificar.hint:='Editar los datos de la instalación';edbuscar.SetFocus;
...
if btnmodificar.Caption = '&Editar' then
  ...
  if ((ckestatus.Checked = true) and (finalizada = 0)) then
    if ckestatus.Checked = true then
      begin
      if application.MessageBox('Estan todos los datos correctos?, dar click en OK para cerrar instalación','Confirmación',mb_yesno+mb_iconInformation) = idyes then
        begin //cerrando instalación
...
{  if ((term=1) and (cierre=0)) then begin
      application.MessageBox('No ,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
  end else begin                                                                                      }
  if ((cierre=1) and (instal=1) and (super=1) and (check =1) and (fin=1))  then
    ...
    if ((instal=0) and (super=0) and (check=0) and (fin=0)) then
      begin
      application.MessageBox('No hay fecha de (instalación,supervicion,finalización), no cumple con los requisitos de checklist,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
      ckhoja.SetFocus;
      exit;
      end;
    if ((instal=0) and (super=0) and (check=0)) then
    begin
       application.MessageBox('No hay fecha de (instalación,supervisión), no cumple con los requisitos de checklist,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       ckhoja.SetFocus;
       exit;
    end;
    if ((instal=0) and (fin=0) and (super=0)) then
    begin
       application.MessageBox('No hay fecha de (instalación,supervisión,finalización),no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       FechaInicio.SetFocus;
       exit;
    end;
    if ((super=0) and (check=0) and (fin=0)) then
    begin
       application.MessageBox('No hay fecha de (supervision,finalización), no cumple con los requisitos de checklist,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       ckhoja.SetFocus;
       exit;
    end;
    if ((instal=0) and (super=0)) then
    begin
       application.MessageBox('No hay fecha de (instalación,supervisión),no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       FechaInicio.SetFocus;
       exit;
    end;
    if ((instal=0) and (fin=0) and (check=0)) then
    begin
       application.MessageBox('No hay fecha de (instalación,finalización), no cumple con los requisitos de checklist,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       ckhoja.SetFocus;
       exit;
    end;
    if ((instal=0) and (check=0)) then
    begin
       application.MessageBox('No hay fecha de (instalación),no cumple con los requisitos de checklist,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       ckhoja.SetFocus;
       exit;
    end;
    if ((super=0) and (check=0)) then
    begin
       application.MessageBox('No hay fecha de (supervisión),no cumple con los requisitos de checklist,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       ckhoja.SetFocus;
       exit;
    end;
    if ((super=0) and (fin=0)) then
    begin
       application.MessageBox('No hay fecha de (supervisión,finalización), no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       FechaFin.SetFocus;
       exit;
    end;
    if ((check=0) and (fin=0)) then
    begin
       application.MessageBox('No hay fecha de (finalización), no cumple con los requisitos de checklist,no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       ckhoja.SetFocus;
       exit;
    end;
    if ((instal=0)) then
    begin
       application.MessageBox('No hay fecha de (instalación),no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       FechaInicio.SetFocus;
       exit;
    end;
    if (check=0) then
    begin
       application.MessageBox('No cumple con los requisitos de checklist, no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       ckhoja.SetFocus;
       exit;
    end;
    if ((fin=0)) then
    begin
       application.MessageBox('No hay fecha de (finalización),no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       FechaFin.SetFocus;
       exit;
    end;
    if ((super=0)) then
    begin
       application.MessageBox('No hay fecha de (supervisión),no puedes cerrar la instalación','Confirmación',mb_IconInformation);
       FechaSuper.SetFocus;
       exit;
    end;
   end;
