object frmMantManuales: TfrmMantManuales
  Left = 270
  Top = 171
  Width = 600
  Height = 445
  Caption = 'Mantenimientos Manuales'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnKeyUp = FormKeyUp
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object RzLabel1: TRzLabel
    Left = 8
    Top = 16
    Width = 37
    Height = 13
    Caption = 'Clave:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel2: TRzLabel
    Left = 8
    Top = 48
    Width = 48
    Height = 13
    Caption = 'Nombre:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel3: TRzLabel
    Left = 8
    Top = 80
    Width = 32
    Height = 13
    Caption = 'Ruta:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel4: TRzLabel
    Left = 8
    Top = 100
    Width = 31
    Height = 13
    Caption = 'Area:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel5: TRzLabel
    Left = 504
    Top = 100
    Width = 53
    Height = 13
    Caption = 'Posicion:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel6: TRzLabel
    Left = 208
    Top = 100
    Width = 51
    Height = 13
    Caption = 'Subtitulo'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel7: TRzLabel
    Left = 8
    Top = 152
    Width = 63
    Height = 13
    Caption = 'Existentes:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object edclave: TRzButtonEdit
    Left = 56
    Top = 8
    Width = 97
    Height = 21
    Color = clWhite
    DisabledColor = clWhite
    FrameHotTrack = True
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 0
    OnChange = edclaveChange
    OnKeyPress = edclaveKeyPress
    ButtonKind = bkFind
    FlatButtons = True
    FlatButtonColor = clWhite
    OnButtonClick = edclaveButtonClick
  end
  object ednombre: TRzDBEdit
    Left = 64
    Top = 40
    Width = 425
    Height = 21
    DataSource = dsmanual
    DataField = 'nombre'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    FrameHotTrack = True
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 2
    OnKeyPress = ednombreKeyPress
  end
  object rgnivel: TRzDBRadioGroup
    Left = 496
    Top = 40
    Width = 89
    Height = 57
    DataField = 'nivel'
    DataSource = dsmanual
    Items.Strings = (
      'Subtitulo'
      'Manual')
    Values.Strings = (
      '1'
      '2')
    Caption = ' Nivel '
    Color = clWhite
    Enabled = False
    ItemHotTrack = True
    TabOrder = 3
    OnClick = rgnivelClick
  end
  object Gsubtitulo: TcxGrid
    Left = 8
    Top = 168
    Width = 577
    Height = 200
    TabOrder = 8
    object Vsubtitulo: TcxGridDBTableView
      DataController.DataSource = dssubtitulo
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      NavigatorButtons.ConfirmDelete = False
      OptionsData.CancelOnExit = False
      OptionsData.Deleting = False
      OptionsData.DeletingConfirmation = False
      OptionsData.Editing = False
      OptionsData.Inserting = False
      object cposicion: TcxGridDBColumn
        Caption = 'Posicion'
        Width = 91
        DataBinding.FieldName = 'posicion'
      end
      object cnombre: TcxGridDBColumn
        Caption = 'Nombre'
        SortOrder = soAscending
        Width = 482
        DataBinding.FieldName = 'nombre'
      end
    end
    object Lsubtitulo: TcxGridLevel
      GridView = Vsubtitulo
    end
  end
  object navegador: TRzDBNavigator
    Left = 232
    Top = 8
    Width = 352
    Height = 25
    HelpType = htKeyword
    DataSource = dsmanual
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    Enabled = False
    TabOrder = 1
    OnClick = navegadorClick
    Color = clWhite
  end
  object btnalta: TRzBitBtn
    Left = 8
    Top = 376
    Width = 120
    Height = 30
    Caption = '&Alta'
    HotTrack = True
    TabOrder = 9
    OnClick = btnaltaClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object btnborrar: TRzBitBtn
    Left = 165
    Top = 376
    Width = 120
    Height = 30
    Caption = '&Borrar'
    Enabled = False
    HotTrack = True
    TabOrder = 10
    OnClick = btnborrarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8091717E8E8E8E8E8E8E8E8E8E8E8E8E856ACAC}
    NumGlyphs = 2
  end
  object btnmodificar: TRzBitBtn
    Left = 314
    Top = 376
    Width = 120
    Height = 30
    Caption = '&Editar'
    Enabled = False
    HotTrack = True
    TabOrder = 11
    OnClick = btnmodificarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object btncancelar: TRzBitBtn
    Left = 464
    Top = 376
    Width = 120
    Height = 30
    Caption = '&Cerrar'
    HotTrack = True
    TabOrder = 12
    OnClick = btncancelarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object cbsubtitulo: TRzComboBox
    Left = 208
    Top = 120
    Width = 281
    Height = 21
    CharCase = ecUpperCase
    Ctl3D = False
    DisabledColor = clWhite
    Enabled = False
    FlatButtonColor = clWhite
    FlatButtons = True
    FrameHotTrack = True
    FrameVisible = True
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 6
    OnKeyPress = cbsubtituloKeyPress
    OnNotInList = cbsubtituloNotInList
    OnSelect = cbsubtituloSelect
  end
  object cbarea: TRzComboBox
    Left = 8
    Top = 120
    Width = 193
    Height = 21
    Ctl3D = False
    DisabledColor = clWhite
    Enabled = False
    FlatButtonColor = clWhite
    FlatButtons = True
    FrameHotTrack = True
    FrameVisible = True
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 5
    OnClick = cbareaClick
    OnKeyPress = cbareaKeyPress
    OnNotInList = cbareaNotInList
    OnSelect = cbareaSelect
  end
  object edruta: TRzButtonEdit
    Left = 48
    Top = 76
    Width = 441
    Height = 21
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    FrameHotTrack = True
    FrameVisible = True
    TabOrder = 4
    OnKeyPress = edrutaKeyPress
    ButtonKind = bkFolder
    FlatButtons = True
    FlatButtonColor = clWhite
    OnButtonClick = edrutaButtonClick
  end
  object edposicion: TRzSpinEdit
    Left = 504
    Top = 120
    Width = 81
    Height = 21
    FlatButtonColor = clWhite
    DisabledColor = clWhite
    Enabled = False
    FlatButtons = True
    FrameHotTrack = True
    FrameVisible = True
    TabOrder = 7
    OnKeyPress = edposicionKeyPress
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    FetchAll = True
    Left = 160
    Top = 232
  end
  object tblmanual: TMyTable
    TableName = 'MantManuales'
    Connection = DMAccesos.conexion
    BeforePost = tblmanualBeforePost
    AfterPost = tblmanualAfterPost
    FetchAll = True
    Left = 216
    Top = 232
  end
  object dsmanual: TDataSource
    DataSet = tblmanual
    Left = 216
    Top = 288
  end
  object sqlsubtitulo: TMyQuery
    Connection = DMAccesos.conexion
    FetchAll = True
    Left = 272
    Top = 232
  end
  object dssubtitulo: TDataSource
    DataSet = sqlsubtitulo
    Left = 272
    Top = 288
  end
  object ODArchivo: TRzOpenDialog
    Title = 'Buscar Manual'
    Filter = 
      'Archivos PDF|*.pdf|Archivos Word|*.doc|Archivos Excel|*.xls|Arch' +
      'ivos JPG|*.jpg|Archivos GIF|*.gif|Archivos PNG|*.png|Todos los A' +
      'rchivos|*.*'
    DefaultExt = 'pdf'
    Left = 160
    Top = 288
  end
end
