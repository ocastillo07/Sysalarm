...
type
  Tfrmboletas_mensuales = class(TForm)
    btncerrar: TRzBitBtn;
...
var
  frmboletas_mensuales: Tfrmboletas_mensuales;
...
{$R *.dfm}
function Tfrmboletas_mensuales.cobro(idcliente,mes:integer):double;
begin
result:=33.22
end;


procedure Tfrmboletas_mensuales.edbuscarButtonClick(Sender: TObject);
begin
      //BUSCAR POR CATALOGO//
...
procedure Tfrmboletas_mensuales.edbuscar2ButtonClick(Sender: TObject);
begin
      //BUSCAR POR CATALOGO//
  frmbusquedas.query:='Select idcliente,if(nombre<>'''' and apaterno<>'''' and amaterno<>'''',concat(nombre,'' '',apaterno,'' '',amaterno),nombre) as Nombre,rsocial AS Razon_Social,dir as Direccion,numero as Numero,ciudad as Ciudad, ' +
                      ' estado as Estado,usuario as Usuario,fecha as Fecha,hora as Hora From clientes order by idcliente';
...
procedure Tfrmboletas_mensuales.edbuscarKeyPress(Sender: TObject;
  var Key: Char);
begin
   if (key=#13)then
   begin
...
procedure Tfrmboletas_mensuales.edbuscar2KeyPress(Sender: TObject;
  var Key: Char);
begin
   if (key=#13)then
   begin
...
procedure Tfrmboletas_mensuales.FormClose(Sender: TObject;
...
procedure Tfrmboletas_mensuales.edfechaKeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#13) then
  begin
...
procedure Tfrmboletas_mensuales.btnaltaClick(Sender: TObject);
   var i, c, mes, hasta, cliente1, mesesbon, contador : integer;
   fechavence, pedido, fechacreacion, year, costo, origen, endolaresstring, idvendedor, idpedido1, pedtmp, cadenames,temp,fecha,monito,tipocont:string;
      ...
      begin
        ...
        // Mensualidad(cliente1, mesesbon, hasta, tipocont, fechatemp,  bonif, tipocambio);
         ...
         begin
            ...
            querys('SELECT distinct pedidos.idpedido,pedidos.concepto as Concepto, '+
             'boletas.CodBarras,boletas.CodBanco,boletas.CodBBVA, boletas.CodBanorte,'+
             'boletas.FechaLarga,boletas.idcliente,boletas.cargo, ' +
             ' ClientesFact.NombreCres as nombre, "'+GetConfiguraciones('convenioBBVA')+'" as convenio FROM  boletas Left join clientes c on c.idcliente =boletas.idcliente '+
             'LEFT JOIN ClientesFact ON (ClientesFact.idfact = c.idfact) left join pedidos on pedidos.idpedido = boletas.idpedido '+
             ' WHERE boletas.idcliente = '+inttostr(cliente1)+' and boletas.FechaVence >='+quotedstr(datetostr(edfecha.Date))+' and tipo = ''M'' order by boletas.FechaVence',0,dmreportes.sqlgeneral);
           // frmmensaje.Close;
...
procedure Tfrmboletas_mensuales.FormShow(Sender: TObject);
var fecha:string;
i : integer;
begin
...
procedure Tfrmboletas_mensuales.btncerrarClick(Sender: TObject);
begin
close;
end;

procedure Tfrmboletas_mensuales.btnocultarClick(Sender: TObject);
begin
//sp1.Visible:=false;
end;

procedure Tfrmboletas_mensuales.chkult_mesClick(Sender: TObject);
begin
 if chkult_mes.checked = false then
   eddias.enabled:=true
 else
...
procedure Tfrmboletas_mensuales.edfechaClick(Sender: TObject);
begin
edmeses.Text:=inttostr(13 - monthof(edfecha.Date));
end;

procedure Tfrmboletas_mensuales.edfechaChange(Sender: TObject);
begin
edmeses.Text:=inttostr(13 - monthof(edfecha.Date));
end;

procedure Tfrmboletas_mensuales.chkgenbolClick(Sender: TObject);
begin
chkpreview.enabled := chkgenbol.Checked;
end;

procedure Tfrmboletas_mensuales.mm1Change(Sender: TObject);
begin
lbltotal1.Caption := 'Total: '+inttostr(mm1.Lines.Count);
end;

procedure Tfrmboletas_mensuales.mm3Change(Sender: TObject);
begin
lbltotal2.Caption := 'Total: '+inttostr(mm3.Lines.Count);
end;

procedure Tfrmboletas_mensuales.mmctacateChange(Sender: TObject);
begin
lbltotal3.Caption := 'Total: '+inttostr(mmctacate.Lines.Count);
end;

procedure Tfrmboletas_mensuales.TimerTimer(Sender: TObject);
begin
timer.enabled := false;
sleep(20000);
timer.Enabled := true;
