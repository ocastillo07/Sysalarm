object frmpuestos: Tfrmpuestos
  Left = 328
  Top = 132
  Width = 523
  Height = 304
  Hint = 'Pantalla ABC de Puestos'
  AutoSize = True
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'ABC de Puestos'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  ShowHint = True
  Visible = True
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnKeyUp = FormKeyUp
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lblclave: TRzLabel
    Left = 0
    Top = 10
    Width = 37
    Height = 13
    Caption = 'Clave:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel1: TRzLabel
    Left = 8
    Top = 38
    Width = 48
    Height = 13
    Caption = 'Nombre:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel2: TRzLabel
    Left = 8
    Top = 94
    Width = 72
    Height = 13
    Caption = 'Descripci'#243'n:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object lblarea: TRzLabel
    Left = 8
    Top = 143
    Width = 31
    Height = 13
    Caption = 'Area:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel3: TRzLabel
    Left = 298
    Top = 37
    Width = 127
    Height = 13
    Caption = 'Unidades de Negocio:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object lbufh: TRzLabel
    Left = 0
    Top = 212
    Width = 13
    Height = 13
    Caption = '...'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMoneyGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object btnalta: TRzBitBtn
    Left = 46
    Top = 236
    Width = 100
    Height = 30
    Hint = 'Dar de Alta a un Puesto'
    ModalResult = 1
    Caption = '&Alta'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    OnClick = btnaltaClick
    Glyph.Data = {
      
      FFFFFF777777777FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ImageIndex = 5
    NumGlyphs = 2
  end
  object btnborrar: TRzBitBtn
    Left = 167
    Top = 236
    Width = 100
    Height = 30
    Hint = 'Elimina el Puesto actual'
    ModalResult = 1
    Caption = '&Borrar'
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 5
    OnClick = btnborrarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8091717E8E8E8E8E8E8E8E8E8E8E8E8E856ACAC}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btnmodificar: TRzBitBtn
    Left = 285
    Top = 236
    Width = 100
    Height = 30
    Hint = 'Editar los datos del Puesto Actual'
    ModalResult = 1
    Caption = '&Editar'
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 4
    OnClick = btnmodificarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    ImageIndex = 2
    NumGlyphs = 2
  end
  object btncancelar: TRzBitBtn
    Left = 407
    Top = 236
    Width = 100
    Height = 30
    Hint = 'Cerrar esta Pantalla'
    Caption = '&Cerrar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    HotTrack = True
    ParentFont = False
    TabOrder = 6
    OnClick = btncancelarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object ednombre: TRzDBEdit
    Left = 14
    Top = 53
    Width = 281
    Height = 21
    Hint = 'Nombre del Puesto'
    DataSource = DSpuestos
    DataField = 'nombre'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FocusColor = clSkyBlue
    FrameColor = clActiveCaption
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 0
    OnKeyPress = ednombreKeyPress
  end
  object eddescripcion: TRzDBEdit
    Left = 10
    Top = 109
    Width = 285
    Height = 21
    Hint = 'Descripci'#243'n del Puesto'
    DataSource = DSpuestos
    DataField = 'descripcion'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    FocusColor = clSkyBlue
    FrameColor = clActiveCaption
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 1
    OnKeyPress = eddescripcionKeyPress
  end
  object cboarea: TRzComboBox
    Left = 13
    Top = 159
    Width = 282
    Height = 21
    Hint = 'Nombre del Area a la que pertenece este puesto'
    CharCase = ecUpperCase
    Ctl3D = False
    DisabledColor = clWhite
    Enabled = False
    FlatButtons = True
    FrameColor = clActiveCaption
    FrameSides = [sdBottom]
    FrameVisible = True
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 2
    OnKeyPress = cboareaKeyPress
    OnNotInList = cboareaNotInList
  end
  object navegador: TRzDBNavigator
    Left = 205
    Top = 0
    Width = 284
    Height = 33
    DataSource = DSpuestos
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    Hints.Strings = (
      'Primer Puesto'
      'Anterior Puesto'
      'Siguiente Puesto'
      'Ultimo Puesto')
    TabOrder = 7
    OnClick = navegadorClick
    Color = clWhite
  end
  object edclave: TRzButtonEdit
    Left = 48
    Top = 5
    Width = 86
    Height = 22
    Hint = 'Buscar en el catalogo'
    DisabledColor = clWhite
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    FocusColor = 16776176
    FrameColor = clActiveCaption
    FrameHotColor = clFuchsia
    FrameSides = [sdBottom]
    FrameVisible = True
    ParentFont = False
    TabOrder = 8
    OnChange = edclaveChange
    OnKeyPress = edclaveKeyPress
    ButtonKind = bkFind
    ButtonShortCut = 119
    FlatButtons = True
    OnButtonClick = edclaveButtonClick
  end
  object ltUnidadNegocio: TRzCheckList
    Left = 318
    Top = 56
    Width = 185
    Height = 169
    DisabledColor = clWhite
    Enabled = False
    FrameHotTrack = True
    FrameVisible = True
    ItemHeight = 17
    TabOrder = 9
  end
  object DSpuestos: TDataSource
    DataSet = tblpuestos
    Left = 416
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    SQL.Strings = (
      'select telefono,tipo_tel from telefonos where idtel=:tel')
    Options.FullRefresh = True
    FetchAll = True
    Left = 272
    ParamData = <
      item
        DataType = ftInteger
        Name = 'tel'
      end>
  end
  object tblpuestos: TMyTable
    TableName = 'puestos'
    Connection = DMAccesos.conexion
    AfterOpen = tblpuestosAfterOpen
    BeforeClose = tblpuestosBeforeClose
    BeforePost = tblpuestosBeforePost
    AfterScroll = tblpuestosAfterScroll
    FetchAll = True
    Left = 344
    object tblpuestosidpuesto: TIntegerField
      AutoGenerateValue = arAutoInc
      FieldName = 'idpuesto'
      Origin = 'puestos.idpuesto'
    end
    object tblpuestosnombre: TStringField
      FieldName = 'nombre'
      Origin = 'puestos.nombre'
      Size = 50
    end
    object tblpuestosdescripcion: TStringField
      FieldName = 'descripcion'
      Origin = 'puestos.descripcion'
      Size = 255
    end
    object tblpuestosidarea: TIntegerField
      FieldName = 'idarea'
      Origin = 'puestos.idarea'
    end
    object tblpuestosusuario: TStringField
      FieldName = 'usuario'
      Size = 30
    end
    object tblpuestoshora: TStringField
      FieldName = 'hora'
      Size = 15
    end
    object tblpuestosfecha: TStringField
      FieldName = 'fecha'
      Size = 15
    end
    object tblpuestosUnidadNegocio: TStringField
      FieldName = 'UnidadNegocio'
      Size = 30
    end
  end
end
