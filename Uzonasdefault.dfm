object frmZonasDefault: TfrmZonasDefault
  Left = 335
  Top = 216
  Width = 441
  Height = 206
  AutoSize = True
  Caption = 'ABC de Zonas Default'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnKeyUp = FormKeyUp
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object RzLabel1: TRzLabel
    Left = 8
    Top = 8
    Width = 37
    Height = 13
    Caption = 'Clave:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel2: TRzLabel
    Left = 193
    Top = 48
    Width = 30
    Height = 13
    Caption = 'Tipo:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel3: TRzLabel
    Left = 306
    Top = 48
    Width = 34
    Height = 13
    Caption = 'Zona:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel4: TRzLabel
    Left = 8
    Top = 72
    Width = 72
    Height = 13
    Caption = 'Descripcion:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object RzLabel5: TRzLabel
    Left = 8
    Top = 48
    Width = 59
    Height = 13
    Caption = 'Protocolo:'
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object lbufh: TRzLabel
    Left = 10
    Top = 120
    Width = 13
    Height = 13
    Caption = '...'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMoneyGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object clave: TRzButtonEdit
    Left = 48
    Top = 8
    Width = 113
    Height = 21
    CharCase = ecUpperCase
    DisabledColor = clWhite
    FocusColor = 16776176
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 0
    OnChange = claveChange
    OnKeyPress = claveKeyPress
    ButtonKind = bkFind
    ButtonShortCut = 119
    FlatButtons = True
    FlatButtonColor = clWhite
    OnButtonClick = claveButtonClick
  end
  object btnalta: TRzBitBtn
    Left = 0
    Top = 138
    Width = 90
    Height = 30
    Caption = '&Alta'
    Color = clWhite
    HotTrack = True
    TabOrder = 6
    OnClick = btnaltaClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object btnborrar: TRzBitBtn
    Left = 114
    Top = 138
    Width = 90
    Height = 30
    Caption = '&Borrar'
    Color = clWhite
    Enabled = False
    HotTrack = True
    TabOrder = 7
    OnClick = btnborrarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8091717E8E8E8E8E8E8E8E8E8E8E8E8E856ACAC}
    NumGlyphs = 2
  end
  object btnmodificar: TRzBitBtn
    Left = 228
    Top = 138
    Width = 90
    Height = 30
    Caption = '&Editar'
    Color = clWhite
    Enabled = False
    HotTrack = True
    TabOrder = 8
    OnClick = btnmodificarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object btncancelar: TRzBitBtn
    Left = 334
    Top = 138
    Width = 90
    Height = 30
    Caption = '&Cerrar'
    Color = clWhite
    HotTrack = True
    TabOrder = 9
    OnClick = btncancelarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object navegador: TRzDBNavigator
    Left = 176
    Top = 0
    Width = 216
    Height = 33
    DataSource = DSzonasdefault
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    Enabled = False
    TabOrder = 1
    OnClick = navegadorClick
    Color = clWhite
  end
  object descripcion: TRzDBEdit
    Left = 8
    Top = 88
    Width = 417
    Height = 21
    DataSource = DSzonasdefault
    DataField = 'nota'
    CharCase = ecUpperCase
    DisabledColor = clWhite
    Enabled = False
    FocusColor = 16776176
    FrameSides = [sdBottom]
    FrameVisible = True
    TabOrder = 5
    OnKeyPress = descripcionKeyPress
  end
  object cbtipo: TRzDBComboBox
    Left = 233
    Top = 48
    Width = 65
    Height = 21
    DataField = 'atid'
    DataSource = DSzonasdefault
    CharCase = ecUpperCase
    Ctl3D = False
    DisabledColor = clWhite
    DropDownCount = 15
    Enabled = False
    FlatButtonColor = clWhite
    FlatButtons = True
    FrameHotTrack = True
    FrameVisible = True
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 3
    OnKeyPress = cbtipoKeyPress
    OnNotInList = cbtipoNotInList
    OnSelect = cbtipoSelect
  end
  object cbzona: TRzDBComboBox
    Left = 347
    Top = 48
    Width = 70
    Height = 21
    DataField = 'znum'
    DataSource = DSzonasdefault
    CharCase = ecUpperCase
    Ctl3D = False
    DisabledColor = clWhite
    DropDownCount = 15
    Enabled = False
    FlatButtonColor = clWhite
    FlatButtons = True
    FrameHotTrack = True
    FrameVisible = True
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 4
    OnKeyPress = cbzonaKeyPress
    OnNotInList = cbzonaNotInList
  end
  object cbprotocolo: TRzComboBox
    Left = 72
    Top = 48
    Width = 121
    Height = 21
    Ctl3D = False
    DisabledColor = clWhite
    Enabled = False
    FlatButtonColor = clWhite
    FlatButtons = True
    FrameHotTrack = True
    FrameVisible = True
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 2
    OnKeyPress = cbprotocoloKeyPress
    OnNotInList = cbprotocoloNotInList
    OnSelect = cbprotocoloSelect
  end
  object tblzonasdefault: TMyTable
    TableName = 'zonas_default'
    Connection = DMAccesos.conexion
    BeforePost = tblzonasdefaultBeforePost
    FetchAll = True
    Left = 272
    object tblzonasdefaultzdid: TIntegerField
      FieldName = 'zdid'
    end
    object tblzonasdefaultatid: TStringField
      FieldName = 'atid'
      FixedChar = True
      Size = 2
    end
    object tblzonasdefaultznum: TStringField
      FieldName = 'znum'
      FixedChar = True
      Size = 2
    end
    object tblzonasdefaultnota: TStringField
      FieldName = 'nota'
      Size = 255
    end
    object tblzonasdefaultidprotocolo: TIntegerField
      FieldName = 'idprotocolo'
    end
    object tblzonasdefaultusuario: TStringField
      FieldName = 'usuario'
      Size = 30
    end
    object tblzonasdefaultfecha: TStringField
      FieldName = 'fecha'
      Size = 15
    end
    object tblzonasdefaulthora: TStringField
      FieldName = 'hora'
      Size = 15
    end
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    FetchAll = True
    Left = 216
  end
  object qtipo: TMyQuery
    Connection = DMAccesos.conexion
    SQL.Strings = (
      'select * from protocolos')
    FetchAll = True
    Left = 112
    Top = 88
  end
  object DSzonasdefault: TDataSource
    DataSet = tblzonasdefault
    Left = 328
  end
end
