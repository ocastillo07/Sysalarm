...
type
  TfrmpedidosAgregar = class(TForm)
...
var
  frmpedidosAgregar: TfrmpedidosAgregar;
  tecla:char;
implementation
...
constructor tfrmpedidosAgregar.inicializa(pedido : string);
begin
chtecla:=#13;
...
function tfrmpedidosAgregar.ValidarCliente_DatosFacturacion:integer;
begin
...
function tfrmpedidosAgregar.ValidarCliente_RemisionarFacturar:integer;
begin
...
procedure TfrmpedidosAgregar.VerificarDatosOportunidad;
begin
with dmreportes do
 begin
...
procedure tfrmpedidosAgregar.MuestraRemiciones(idpedido:string);
...
if sqlgeneral.RecordCount > 0 then
 begin
   ...
   lblidremicion.Caption:='Remicion: '+sqlgeneral.fieldbyname('idremicion').AsString;
   lblFechaAltaRem.Caption:='Fecha Remicion: '+sqlgeneral.fieldbyname('FechaAlta').AsString;
   lblFechaBajaRem.Caption:='Fecha Cancelacion Remicion: '+sqlgeneral.fieldbyname('FechaBaja').AsString;
   lblHoraAltaRem.Caption:='Hora Remicion: '+sqlgeneral.fieldbyname('HoraAlta').AsString;
   lblHoraBajaRem.Caption:='Hora Cancelacion Remicion: '+sqlgeneral.fieldbyname('HoraBaja').AsString;
   lblUsuarioAltaRem.Caption:='Usuario Remicion: '+sqlgeneral.fieldbyname('UsuarioAlta').AsString;
   lblUsuarioBajaRem.Caption:='Usuario Cancelacion Remicion: '+sqlgeneral.fieldbyname('UsuarioBaja').AsString;
...
procedure tfrmpedidosAgregar.VendeSinExistencias(cod_cve:string;cant,pzs:integer);
begin
...
procedure tfrmpedidosAgregar.BorraArticulos(tabla,campo,valor,cod_cve,id:string);
begin
...
procedure tfrmpedidosAgregar.CalculaTotales(query:tMyQuery);
var subtotal,descuento,iva,hrs, hrscom:double;
begin
if query.RecordCount = 0 then
   begin
...
procedure tfrmpedidosAgregar.ModificaArticulo(query:tMyQuery;tabla,campo,valor:string);
begin
...
function tfrmpedidosAgregar.ValidaProducto(CodCve:string):integer;
begin
...
procedure tfrmpedidosAgregar.InsertaArticulo(query:tMyQuery;tabla,campo,valor,cod_cve:string;cantidad,requisicion,idCardexArt:integer);
...
if rejilla.Enabled = false then
 rejilla.Enabled:=true;
 querys('select articulos.inventariable,articulos.piezasxunidad,articulos.PrecioCapturable,articulos.servicio,articulos.moneda,usuarios.PrecioCapturable as pc from articulos,usuarios where cod_cve='+quotedstr(cod_cve)+' and usuarios.idusuario='+inttostr(frmprincipal.ucprincipal.CurrentUser.UserID),0,sqlgeneral);
...
procedure tfrmpedidosAgregar.InsertaArticulosPaquete(tabla,campo,valor,cod_paquete:string;query:tmyquery;cantidad:integer);
...
//    LockArt(cod_cve);
    ...
    querys('update articulos set apartado=apartado+'+inttostr((cant*pzsxunidad)+pzs)+',existencia=existencia-'+inttostr((cant*pzsxunidad)+pzs)+' where cod_cve='+quotedstr(cod_cve),1,sqltemp);
    sqlgeneral.Next;
   end;
query.Close;
...
procedure tfrmpedidosAgregar.LimpiaDatArt;
begin
     //limpia los campos
     gbarticulo.Color := clwhite;
     gbarticulo.Hint := '';
...
function tfrmpedidosAgregar.ValidaExistencias(CodCve:string;cantidad:integer;piezas:boolean):integer;
var
...
procedure tfrmpedidosAgregar.TraePedido(idpedido:integer);
var
  i : integer;
  sqltemp1 : tmyquery;
begin
...
sqlgeneral.SQL.Add('SELECT p.idinstal,u.nombre,u.apaterno,u.amaterno,o.nombre AS origen,un.nombre AS UN,p.idpedido,p.idpresupuesto,p.idoportunidad,p.idcliente,p.idvendedor,p.NotaInst, '+
                   ...
                   //INNER JOIN tipo_oportunidad ON (pedidos.idTipoOportunidad = tipo_oportunidad.idtipo_oportunidad)
sqlgeneral.Open;

//trae el nombre de la linea
//querys('select nombre from lineas where idlinea='+sqlgeneral.fieldbyname('idlinea').Asstring,0,frmprincipal.SqlGeneral);
//cboLinea.text := frmprincipal.SqlGeneral.fieldbyname('nombre').AsString;
...
if (sqlgeneral.fieldbyname('TotHrsCom').AsFloat > 0 ) then
  ...
  {
if sqlgeneral.fieldbyname('instpend').Asinteger = 0 then
  ckinstpend.Checked := false
else
  ckinstpend.Checked := true;
   }
...
//edFechaInstalacion.Text := sqlgeneral.fieldbyname('FechaInstalacion').Asstring;
...
//edTipoOportunidad.Text:=sqlgeneral.FieldByName('idTipoOportunidad').Asstring;
edTipoOportunidad.Text:=sqlgeneral.FieldByName('idUnidadNegocio').Asstring;
//cbonom_TOportunidad.Text:=sqlgeneral.FieldByName('oportunidad').Asstring;
...
procedure tfrmpedidosAgregar.limpia;
begin
    REConcepto.Clear;
    RENotaInst.Clear;
    btnremisionar.Tag:=0;
...
procedure tfrmpedidosAgregar.VendeConBackOrder(art:string;cant,pzs:integer);
...
querys('select apartado,ordenados,existencia,piezasxunidad,backorder from articulos where cod_cve='+quotedstr(art),0,sqlgeneral);
...
if dif < 0 then   //if application.MessageBox(pchar('El articulo con clave '+art+' cuenta con '+inttostr(existencias div pzsxunidad)+' Unidades, '+inttostr(existencias mod pzsxunidad)+' Piezas, '+sqlgeneral.fieldbyname('backorder').Asstring+' en BackOrder y usted esta pidiendo '+inttostr(cant)+' Unidades y '+inttostr(pzs)+' Piezas, desea crear BackOrder?'),'Aviso',mb_yesno)=idyes then
  ...
  if existencias = 0 then
    ...
    querys('update articulos set backorder=backorder+'+inttostr((cant*pzsxunidad)+pzs)+' where cod_cve='+quotedstr(art),1,sqlgeneral);
     // try
    ...
    querys('update articulos set existencia=0, backorder=backorder+('+inttostr(abs(dif))+') where cod_cve='+quotedstr(art),1,sqlgeneral);
...
function tfrmpedidosAgregar.TraePresupuesto(idpresupuesto:string):integer;
var estatus,art,nomart,precio:string;   //  ,top
...
procedure tfrmpedidosAgregar.LimpiaTotales;
begin
edTotMat.Text:='0';
edTotHrs.Text:='0';
edTotHrsCom.Text:='0';
...
procedure TfrmpedidosAgregar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
//UnLockArt('',true);
...
procedure TfrmpedidosAgregar.bonificacion(mes:string);
begin
...
procedure TfrmpedidosAgregar.btnaltaClick(Sender: TObject);
...
//0=no surtido 1=surtido 2=cancelado
   ...
   begin
      ...
      begin
         ...
         if (ednum_origen.Text = '') or (cbonom_TOportunidad.Text = '') then
          begin
...
//*******************************************************************************************\\
         ...
         {//trae el maximo idpedido + 1
         querys('select max(idpedido) as idpedido from pedidos',0,sqltemp);
         NumPed:=sqltemp.Fields.Fields[0].Asinteger+1;
         querys('select idmovi from TipoMov where nombre="PEDIDOS"',0,sqlgeneral);

         comentado dalila
         //verifica si hay requisiciones
         querys('select * from requisiciones where iddoc='+edNumPedido.Text+' and idmovi='+sqlgeneral.Fields.Fields[0].AsString,0,sqlgeneral);
         if sqlgeneral.RecordCount > 0 then
         begin
            //busca el tipo de moneda
            querys('select moneda from articulos where cod_cve='+quotedstr(sqlgeneral.fieldbyname('cod_cve').AsString),0,sqltemp);
            if sqltemp.FieldByName('moneda').AsString = 'P' then
               tc := 1
            else
               tc := strtofloat(edtcp.Text);
            //insertaen el encabezado del pedido
            querys('insert into pedidos(idpedido,idpresupuesto,idUnidadNegocio,idcliente,idvendedor,FechaCreacion,usuario,fecha,hora,TipoCambio,concepto,estatus,idlinea,idOrigen,autorizo) SELECT '+inttostr(NumPed)+',pedidos.idpresupuesto '+
             ',pedidos.idUnidadNegocio,pedidos.idcliente,pedidos.idvendedor,pedidos.FechaCreacion,'+quotedstr(frmprincipal.ucprincipal.CurrentUser.LoginName)+', pedidos.fecha, pedidos.hora, pedidos.TipoCambio, pedidos.concepto,2,'+inttostr(idlinea)+','+ednum_origen.Text + ','+edCveAutorizo.Text +
             ' FROM pedidos where idpedido='+ednumpedido.text,1,sqltemp);
            //inserta los detalles de este pedido
            querys('insert into DetPedido(idpedido,cant,piezas,cod_cve,utilidad,surtido,idrequisicion,costo,tc,usuario,fecha,hora) select '+inttostr(NumPed)+',if (requisiciones.piezas=1,0,requisiciones.cantidad/articulos.piezasxunidad) as cant,if (requisiciones.piezas=1 '+
             ',requisiciones.cantidad,0) as piezas,requisiciones.cod_cve,'+quotedstr(cboutilidad.Text)+',0,idrequisicion,costo,'+floattostr(tc)+',requisiciones.usuario,requisiciones.fecha,requisiciones.hora from requisiciones inner join articulos on '+
             'requisiciones.cod_cve=articulos.cod_cve inner join TipoMov on requisiciones.idmovi=TipoMov.idmovi where TipoMov.nombre='+quotedstr('PEDIDOS')+' and requisiciones.iddoc='+edNumPedido.Text,1,sqlgeneral);

            //inserta en el pedido original en el campo idPedidoHijo
            querys('update pedidos set idPedidoHijo='+inttostr(NumPed)+' where idpedido='+edNumPedido.Text,1,sqlgeneral);
         end;
...
procedure TfrmpedidosAgregar.edpresupuestoButtonClick(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.edpresupuestoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmpedidosAgregar.CargaCombo(sql,tabla:string;combo : TRzComboBox);
begin
combo.Items.Clear;
...
procedure TfrmpedidosAgregar.FormShow(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.btncancelarClick(Sender: TObject);
...
procedure TfrmpedidosAgregar.edcompaniaKeyPress(Sender: TObject; var Key: Char);
begin
if key=#13 then
begin
  key:=#0;
...
procedure TfrmpedidosAgregar.edNumPedidoButtonClick(Sender: TObject);
begin
 frmbusquedas.query:='SELECT pedidos.idpedido,if(clientes.rsocial is null or clientes.rsocial='''',concat(ifnull(clientes.nombre,''''),'' '',ifnull(clientes.apaterno,'' ''),'' '',ifnull(clientes.amaterno,'' '')),clientes.rsocial) as cliente,ClientesFact.rsocial, ' +
                     ...
                     ',''Facturado'')))) as Estatus,pedidos.TotMn,pedidos.FechaCreacion,pedidos.usuario, pedidos.fecha, pedidos.hora FROM  pedidos left JOIN clientes ON (pedidos.idcliente = clientes.idcliente) left join ClientesFact on clientes.idfact = ' +
                     ' ClientesFact.idfact ';
...
procedure TfrmpedidosAgregar.edNumPedidoKeyPress(Sender: TObject; var Key: Char);
begin
key:=validanumeros(key,false);
key:=validapunto(ednumpedido.Text+'.',key);
...
procedure TfrmpedidosAgregar.btnmodificarClick(Sender: TObject);
begin
...
procedure tfrmpedidosAgregar.EliminaPedido;
begin
...
procedure TfrmpedidosAgregar.btnborrarClick(Sender: TObject);
begin
{//si tiene anticipos o esta facturada no se podra eliminar
if (edTipoOportunidad.Text = '4') and (strtodate(edFechaCreacion.Text) >= encodedate(yearof(today), 4, 1)) then
if not (((frmprincipal.AreaUsuario = 'COBRANZA') and (frmprincipal.PuestoUsuario = 'GERENTE')) OR (frmprincipal.AreaUsuario = 'INFORMATICA')) then
  begin
...
function TfrmpedidosAgregar.ValidaExistenciasPaquete(codigo:string;cantidad:integer):integer;
var valor:integer;
begin
btnagregar.Tag:=0;
...
procedure TfrmpedidosAgregar.btnremisionarClick(Sender: TObject);
...
if
 ValidarCliente_RemisionarFacturar = 1 then
   exit;
...
procedure TfrmpedidosAgregar.sqlrejillaTempAfterOpen(DataSet: TDataSet);
begin
...
procedure TfrmpedidosAgregar.btnparcializarClick(Sender: TObject);
...
procedure TfrmpedidosAgregar.edNumPedidoChange(Sender: TObject);
begin
if ednumpedido.Text = '' then
   limpia;
end;

procedure TfrmpedidosAgregar.RzBitBtn2Click(Sender: TObject);
begin
{frmconceptofact.inicializa(strtoint(ednumpedido.text),sqlpedidos.FieldByName('concepto').asstring);
frmconceptofact.Showmodal ;}
end;

procedure TfrmpedidosAgregar.btnImpFactClick(Sender: TObject);
var
  year,dia,mes:word;
  dec,deducible,idfact, cad : string;     //   , cad2
begin
...
// NumToLetras.Numero := strtoint(midstr(edTotMn.Text,1,posex('.',edTotMn.Text)-1));
...
// idfact := sqlgeneral.fieldbyname('idfactura').AsString;
 ...
 querys('select DetPedido.idpedido,DetPedido.id,DetPedido.cod_cve,articulos.nombre,DetPedido.cant,DetPedido.piezas,articulos.desc_esp/100 as descuento,DetPedido.tc,if(articulos.aplica_desc = 1,''TRUE'',''FALSE'') '+
        ' as aplica_desc,(DetPedido.costo*DetPedido.tc) as precio,if(articulos.moneda = ''D'',''DOLARES'',''PESOS'') as Moneda,(if (articulos.aplica_desc = 1,(((DetPedido.costo-(DetPedido.costo*articulos.desc_esp/100)) + ((DetPedido.costo- '+
        ...
        '((DetPedido.costo/articulos.piezasxunidad)*articulos.iva/100))*DetPedido.piezas))*DetPedido.tc) as Total,articulos.piezasxunidad,(articulos.tinstalacion*DetPedido.cant)  as tinstalacion,DetPedido.utilidad,DetPedido.paquete,'+
        'DetPedido.idrequisicion,DetPedido.idCardexArt from DetPedido inner join articulos on DetPedido.cod_cve = articulos.cod_cve where DetPedido.idpedido='+edNumPedido.Text+' order by DetPedido.id',0,sqlgeneral2);
...
//ReNotaInst.Text := sqlgeneral2.SQL.Text;
 ...
 {querys('select DetPedido.id,DetPedido.cod_cve,articulos.nombre,if(DetPedido.cant=0,DetPedido.piezas,DetPedido.cant) as cant,articulos.iva/100 as Iva,articulos.desc_esp/100 as descuento, ' +
        ...
        'articulos.cod_cve where DetPedido.idpedido='+edNumPedido.Text,0,sqlgeneral2);
       ReNotaInst.Text := sqlgeneral2.SQL.Text;}
...
procedure TfrmpedidosAgregar.edcodigoButtonClick(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.edcodigoEnter(Sender: TObject);
begin
if eddescripcion.Text = '' then
  edcantidad.Text:='1';
end;

procedure TfrmpedidosAgregar.edcodigoKeyPress(Sender: TObject; var Key: Char);
begin
if (key=#27) then
 begin
  key:=#0;
...
procedure TfrmpedidosAgregar.edcantidadChange(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.edcantidadKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmpedidosAgregar.btnagregarClick(Sender: TObject);
...
procedure TfrmpedidosAgregar.RejillaDBTableView1DblClick(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.btnrecalcularClick(Sender: TObject);
begin
if btnalta.Caption='&Guardar' then
   CalculaTotales(SqlRejillaAlta);
if btnmodificar.caption = '&Guardar' then
...
procedure TfrmpedidosAgregar.edpresupuestoChange(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.edcvecteKeyPress(Sender: TObject; var Key: Char);
...
procedure TfrmpedidosAgregar.edcvecteKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
Var r : integer;
begin
...
procedure TfrmpedidosAgregar.edcvecteChange(Sender: TObject);
begin
if btnalta.Caption = '&Guardar' then
 begin
  if edcvecte.Text = '' then
...
procedure TfrmpedidosAgregar.DescPPExit(Sender: TObject);
begin
{if DescPP.Text <> '' then
   begin
...
procedure TfrmpedidosAgregar.edAnticipoExit(Sender: TObject);
begin
if strtofloat(edanticipo.Text)>strtofloat(edTotMn.Text) then
 begin
...
procedure TfrmpedidosAgregar.SqlrejillaaltaAfterOpen(DataSet: TDataSet);
begin
CalculaTotales(SqlRejillaAlta);
lbltotArts.Caption :='Total de Articulos ' + inttostr(dataset.RecordCount);
end;

procedure TfrmpedidosAgregar.sqlrejillaeditAfterOpen(DataSet: TDataSet);
begin
CalculaTotales(SqlRejillaedit);
lbltotArts.Caption := 'Total de Articulos '+ inttostr(dataset.RecordCount);
end;

procedure TfrmpedidosAgregar.edCHPChange(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.DescPromChange(Sender: TObject);
begin
{try
...
procedure TfrmpedidosAgregar.DescPromExit(Sender: TObject);
begin
{if DescPP.Text <> '' then
   begin
    DescPP.Text:=trim(Format('%8.2f',[strtofloat(DescPP.Text)]));
...
procedure TfrmpedidosAgregar.DescPromKeyPress(Sender: TObject; var Key: Char);
begin
{key:=ValidaNumeros(key,false);
...
procedure TfrmpedidosAgregar.DescPPChange(Sender: TObject);
begin
{try
strtofloat(DescPP.text);
except
...
procedure TfrmpedidosAgregar.DescPPKeyPress(Sender: TObject; var Key: Char);
begin
{key:=ValidaNumeros(key,false);
key:=ValidaPunto(DescPP.Text,key);
if (key=#13) and (eddescuento.Text<>'') then
...
procedure TfrmpedidosAgregar.edAnticipoChange(Sender: TObject);
begin
{ try
...
procedure TfrmpedidosAgregar.edAnticipoKeyPress(Sender: TObject; var Key: Char);
begin
key:=ValidaNumeros(key,false);
//key:=ValidaPunto(edanticipo.Text,key);
end;

procedure TfrmpedidosAgregar.RejillaDBTableView1KeyUp(Sender: TObject;
...
procedure TfrmpedidosAgregar.RejillaDBTableView1KeyPress(Sender: TObject;
  var Key: Char);
  begin
...
procedure TfrmpedidosAgregar.edcodigoChange(Sender: TObject);
begin
if edcodigo.Text = '' then
LimpiaDatArt;
end;

procedure TfrmpedidosAgregar.FormCloseQuery(Sender: TObject;
...
procedure TfrmpedidosAgregar.edcvecteButtonClick(Sender: TObject);
var r : integer;
sqltemp1 : tmyquery;
begin
sqltemp1:=tmyquery.Create(self);
...
procedure TfrmpedidosAgregar.edTipoNegButtonClick(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.edTipoNegKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmpedidosAgregar.cbonom_vendClick(Sender: TObject);
begin
//if edcve_vend.Text = '' then
...
procedure TfrmpedidosAgregar.Edcve_vendKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmpedidosAgregar.ednum_origenKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmpedidosAgregar.cbonom_origenClick(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.cbonom_origenChange(Sender: TObject);
begin
if cbonom_origen.Text = '' then
ednum_origen.Text:='';
end;

procedure TfrmpedidosAgregar.ednum_origenChange(Sender: TObject);
begin
if ednum_origen.Text = '' then
 cbonom_origen.Text:='';
end;

procedure TfrmpedidosAgregar.Edcve_vendChange(Sender: TObject);
begin
if edcve_vend.Text = '' then
cbonom_vend.Text:='';
end;

procedure TfrmpedidosAgregar.cbonom_vendChange(Sender: TObject);
begin
if cbonom_vend.Text = '' then
 edcve_vend.Text:='';
end;

procedure TfrmpedidosAgregar.edTipoOportunidadChange(Sender: TObject);
begin
if edTipoOportunidad.Text = '' then
 cbonom_TOportunidad.Text:='';
end;

procedure TfrmpedidosAgregar.edTipoOportunidadKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmpedidosAgregar.cbonom_TOportunidadClick(Sender: TObject);
begin
{
...
procedure TfrmpedidosAgregar.cbonom_vendKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
begin
 key := #0;
...
procedure TfrmpedidosAgregar.cbonom_origenKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then
 begin
...
procedure TfrmpedidosAgregar.cbonom_TOportunidadKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then
begin
...
procedure TfrmpedidosAgregar.edTCPKeyPress(Sender: TObject; var Key: Char);
begin
key:=validanumeros(key,false);
if key = #13 then
 begin
...
procedure TfrmpedidosAgregar.edTCPExit(Sender: TObject);
begin
if edTCP.Text = '' then
 begin
...
procedure TfrmpedidosAgregar.edCHPKeyPress(Sender: TObject; var Key: Char);
begin
key:=validanumeros(key,false);
end;

procedure TfrmpedidosAgregar.edCHPExit(Sender: TObject);
begin
if edCHP.Text = '' then
 begin
...
procedure TfrmpedidosAgregar.btnfacturarClick(Sender: TObject);
var pzsxunidad,pzs,cant,idmovi,cont,parc:integer;
    remicionado : boolean;
    cod_cve,fact,idfactura,idremicion,idinst : string;
    tot:double;    //  total,tc,ant,
begin
//CSI
...
idinst:=edinstalacion.text;
cont:=0; btnparcializar.Tag := 0;
 try
if sqlpedidos.State = dsedit then
 sqlpedidos.Post;
  //verifica de que tipo de facturas puede elaborar este usuario
 {querys('select FacturaP,FacturaD,FacturaC from usuarios where idusuario = '+inttostr(frmprincipal.ucprincipal.CurrentUser.UserID),0,sqlgeneral);
  if sqlgeneral.Fields.Fields[0].Asboolean then
   begin
    inc(cont);
    fact := 'FacturaP';
   end;
  if sqlgeneral.Fields.Fields[1].Asboolean then
   begin
    inc(cont);
    fact := 'FacturaD';
   end;
  if sqlgeneral.Fields.Fields[2].Asboolean then
   begin
    inc(cont);
    fact := 'FacturaC';
   end;    }
 ...
 { }
  ...
  if cont > 1 then
   ...
   repeat
     ...
     begin
       ...
       querys('select '+BorrarPed.Items.Strings[borrarped.value]+' from folios',0,sqltemp);
       idfactura:=BorrarPed.Items.Strings[borrarped.value][8]+inttostr(sqltemp.fieldbyname(BorrarPed.Items.Strings[borrarped.value]).AsInteger+1);
  ...
  begin
  //if (sqlgeneral.Fields.Fields[0].Asboolean = false) and (sqlgeneral.Fields.Fields[1].Asboolean = false) and (sqlgeneral.Fields.Fields[2].Asboolean= false) then
  if cont = 0 then
     begin
    ...
    begin
     //trae el maximo numero de factura
     querys('select '+fact+' from folios',0,sqltemp);
     idfactura:=fact[8]+inttostr(sqltemp.fieldbyname(fact).AsInteger+1);
     end;
  end;
//verifica si este pedido ya se encuentra remicionado
...
if remicionado = false then
 ...
 ant := strtofloat(edanticipo.Text);}
// tot := strtofloat(edTotDlls.Text);
// tot := strtofloat(format('%8f',[(tc*tot)-ant]));
  tot :=strtofloat(edsaldo.text);
//actualiza el campo ultima compra del cliente al igual que su saldo
bpcambios('clientes', edcuenta.Text, 'idcliente');
 querys('update clientes set UltimaCompra='+quotedstr(datetostr(FechaServidor))+',saldo=saldo+'+floattostr(tot)+',TotComprado=TotComprado+'+edTotMn.Text+' where idcliente='+edcuenta.Text,1,sqltemp);
...
if  btnparcializar.Tag = 0 then
 begin
  Application.CreateForm(TfrmRemiFac, frmRemiFac);
  frmremifac.inicializa(false, remicionado, strtoint(edNumPedido.Text),idfactura);
  frmremifac.ShowModal;
end;
...
if remicionado = false then
 begin
  if (strtofloat(edanticipo.Text) > 0) then
   if strtofloat(edanticipo.Text)=strtofloat(edTotMn.Text) then
    begin
     querys('insert into PagosClientes(movimiento,idcliente,idpedido,idremicion,idfactura,FechaAlta,cargo,abono,TipoPago,TipoCambio,FechaVence,usuario,fecha,hora,restante) values (0,'+edcuenta.Text+','+edNumPedido.Text+','+idremicion+','+quotedstr(idfactura)+','+quotedstr(datetostr(FechaServidor))+','+edTotMn.Text+','+edAnticipo.Text+','+quotedstr('AT')+','+edTCP.Text+','+quotedstr(edFechaVencimiento.Text)+','+quotedstr(frmprincipal.ucprincipal.CurrentUser.LoginName)+','+quotedstr(datetostr(FechaServidor))+','+quotedstr(timetostr(HoraServidor))+',0'+')',1,sqltemp);
     //actualizo el campo saldado
     querys('update pedidos set saldado = 1,FechaLiquidacion='+quotedstr(datetostr(FechaServidor))+',saldo=0 where idpedido ='+edNumPedido.Text,1,sqltemp);
    end
   else  //ANTICIPO MENOR QUE EL TOTAL
    //querys('insert into PagosClientes(movimiento,idcliente,idpedido,idremicion,idfactura,FechaAlta,cargo,abono,TipoPago,FechaVence,TipoCambio,usuario,fecha,hora,restante) values (0,'+edcuenta.Text+','+edNumPedido.Text+','+idremicion+','+quotedstr(idfactura)+','+quotedstr(datetostr(FechaServidor))+','+edTotMn.Text+','+edAnticipo.Text+','+quotedstr('AT')+','+quotedstr(edFechaVencimiento.Text)+','+quotedstr(frmprincipal.ucprincipal.CurrentUser.LoginName)+','+edTCP.Text+','+quotedstr(datetostr(FechaServidor))+','+quotedstr(timetostr(HoraServidor))+','+Format('%8f',[tot])+')',1,sqltemp)
    querys('insert into PagosClientes(movimiento,idcliente,idpedido,idremicion,idfactura,FechaAlta,cargo,abono,TipoPago,FechaVence,usuario,TipoCambio,fecha,hora,restante) values (0,'+edcuenta.Text+','+edNumPedido.Text+','+idremicion+','+quotedstr(idfactura)+','+quotedstr(datetostr(FechaServidor))+','+edTotMn.Text+','+edAnticipo.Text+','+quotedstr('AT')+','+quotedstr(edFechaVencimiento.Text)+','+quotedstr(frmprincipal.ucprincipal.CurrentUser.LoginName)+','+edTCP.Text+','+quotedstr(datetostr(FechaServidor))+','+quotedstr(timetostr(HoraServidor))+','+floattostr(tot)+')',1,sqltemp)
  else   //NO DIO ANTICIPO
   querys('insert into PagosClientes(movimiento,idcliente,idpedido,idremicion,idfactura,FechaAlta,cargo,abono,FechaVence,TipoCambio,usuario,fecha,hora,restante) values (0,'+edcuenta.Text+','+edNumPedido.Text+','+idremicion+','+quotedstr(idfactura)+','+quotedstr(datetostr(FechaServidor))+','+edTotMn.Text+',0,'+quotedstr(edFechaVencimiento.Text)+','+edTCP.Text+','+quotedstr(frmprincipal.ucprincipal.CurrentUser.LoginName)+','+quotedstr(datetostr(FechaServidor))+','+quotedstr(timetostr(HoraServidor))+','+Format('%8.2f',[tot])+')',1,sqltemp);
    ...
    if querys('SELECT FormasPagoPre.parcialidades FROM presupuestos2 left JOIN FormasPagoPre ON (presupuestos2.idopcion = FormasPagoPre.idopcion) WHERE presupuestos2.idpresupuesto ='+edpresupuesto.Text,0,sqlgeneral) > 0 then
     if sqlgeneral.Fieldbyname('parcialidades').AsInteger <> 0 then
      begin
       ...
       querys('select ifnull((sum(ifnull(cargo,0))-sum(ifnull(abono,0))),0) as restante from PagosClientes where idpedido= '+edNumPedido.Text+' and movimiento <> 3',0,sqlgeneral);
       Parcializa(sqlgeneral.fieldbyname('restante').Asfloat,strtofloat(edtcp.Text),FechaServidor,strtoint(ednumpedido.text),parc,strtoint(edcuenta.Text),'M');
       application.MessageBox('Pedido Facturado','Aviso',mb_iconinformation);
      end;
...
//trae el restante de este pedido de la tabla de PagosClientes
//querys('select sum(restante) as r from PagosClientes where idpedido ='+ednumpedido.text,0,sqlgeneral);
//restante := sqlgeneral.fieldbyname('r').AsFloat;
//querys('insert into PagosClientes(movimiento,idcliente,idpedido,idremicion,idfactura,FechaAlta,abono,TipoPago,FechaVence,TipoCambio,usuario,fecha,hora,restante) values (10,'+edcuenta.Text+','+edNumPedido.Text+','+idremicion+','+quotedstr(idfactura)+','+quotedstr(datetostr(FechaServidor))+','+edsaldo.Text+','+quotedstr(cboFormaPago.Text[2]+cboFormaPago.Text[3])+','+quotedstr(edFechaVencimiento.Text)+','+edTCP.Text+','+quotedstr(frmprincipal.ucprincipal.CurrentUser.LoginName)+','+quotedstr(datetostr(FechaServidor))+','+quotedstr(timetostr(HoraServidor))+',-'+Format('%8.2f',[restante])+')',1,sqltemp);
//querys('update pedidos set saldado=1 where idpedido='+ednumpedido.Text,1,sqlgeneral);
...
{    if frmprincipal.ucprincipal.CurrentUser.LoginName = 'dalyla' then
  ...
  querys('update folios set '+fact+'='+idfactura,1,sqltemp);
  dmaccesos.conexion.Commit;
  MuestraRemiciones(edNumPedido.Text);
  btnremisionar.Enabled:=false;
  btnfacturar.Enabled:=false;
...
{if btnmodificar.Caption = '&Guardar' then
  ...
  if reconcepto.Text = '' then
   begin
    dmreportes.ConceptoBoleta.Execute;
    querys('update pedidos set concepto ='+quotedstr(dmreportes.ConceptoBoleta.Text),1,sqlgeneral);
   end;  }
...
procedure TfrmpedidosAgregar.TSpedidoClick(Sender: TObject);
begin
panel2.Parent:= tspedido;
end;

procedure TfrmpedidosAgregar.PCpedidosChange(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.edTipoNegExit(Sender: TObject);
begin
//tecla:=#13;
//edTipoNeg.OnKeyPress(self,tecla);
end;

procedure TfrmpedidosAgregar.edcvecteExit(Sender: TObject);
begin
if edcvecte.Text <> '' then
 begin
...
procedure TfrmpedidosAgregar.cboLineaNotInList(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.cboLineaKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
 begin
  key := #0;
...
procedure TfrmpedidosAgregar.cbonom_vendNotInList(Sender: TObject);
begin
//edcve_vend.Clear;
//cbonom_vend.Text := '';
end;

procedure TfrmpedidosAgregar.cbonom_TOportunidadNotInList(Sender: TObject);
begin
edTipoOportunidad.Clear;
cbonom_toportunidad.Text := '';
end;

procedure TfrmpedidosAgregar.cbonom_origenNotInList(Sender: TObject);
begin
ednum_origen.Clear;
cbonom_origen.Text := '';
end;

procedure TfrmpedidosAgregar.edCveAutorizoChange(Sender: TObject);
begin
if edCveAutorizo.Text = '' then
cboNomAutorizo.Text:='';
end;

procedure TfrmpedidosAgregar.edCveAutorizoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmpedidosAgregar.cboNomAutorizoChange(Sender: TObject);
begin
if cboNomAutorizo.Text = '' then
 edcveAutorizo.Text:='';
end;

procedure TfrmpedidosAgregar.cboNomAutorizoClick(Sender: TObject);
...
procedure TfrmpedidosAgregar.cboNomAutorizoKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then
begin
...
procedure TfrmpedidosAgregar.cboNomAutorizoNotInList(Sender: TObject);
begin
//edcveAutorizo.Clear;
//cbonomAutorizo.Text := '';
end;

procedure TfrmpedidosAgregar.EdPrecioKeyPress(Sender: TObject; var Key: Char);
begin
 if key = #27 then
   begin
     key:=#0;
...
procedure TfrmpedidosAgregar.EdPrecioExit(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.SqlrejillaaltaAfterPost(DataSet: TDataSet);
begin
...
procedure TfrmpedidosAgregar.sqlrejillaeditAfterPost(DataSet: TDataSet);
begin
if dataset.RecordCount = 28 then
 begin
  application.MessageBox('Ya se ha llegado al limite de 28 articulos por pedido y no podra agregar mas','Aviso',mb_iconinformation);
...
procedure TfrmpedidosAgregar.cboLineaClick(Sender: TObject);
begin
//Carga los tipos de Oportunidad
//CargaCombo('select tipo_oportunidad.nombre from tipo_oportunidad inner join lineas on lineas.idlinea = tipo_oportunidad.idlinea where lineas.nombre = '+quotedstr(cboLinea.Text)+' order by tipo_oportunidad.nombre','Tipos de Oportunidad',cbonom_TOportunidad);
end;

procedure TfrmpedidosAgregar.CboUtilidadNotInList(Sender: TObject);
begin
CboUtilidad.ItemIndex := 0;
end;

procedure TfrmpedidosAgregar.edvtasxctaKeyPress(Sender: TObject; var Key: Char);
begin
key := ValidaNumeros(key,false);
...
procedure TfrmpedidosAgregar.edvtasxctaButtonClick(Sender: TObject);
var r : integer;
begin
r :=BuscaCliente(sqlclientes);
if (r <> 0)  then
...
procedure TfrmpedidosAgregar.edFechaVencimientoClick(Sender: TObject);
begin
//showmessage('OnClick');
end;

procedure TfrmpedidosAgregar.edFechaVencimientoChange(Sender: TObject);
begin
//showmessage('OnChange');
end;

procedure TfrmpedidosAgregar.btnregresarClick(Sender: TObject);
var fechaRemi:string;
begin
if (edTipoOportunidad.Text = '4') and (sqlpedidos.FieldByName('FechaCreacion').AsDateTime >= encodedate(yearof(today), 4, 1)) then
  begin
  if not (((frmprincipal.AreaUsuario = 'COBRANZA') and (frmprincipal.PuestoUsuario = 'GERENTE'))
     OR (frmprincipal.AreaUsuario = 'SISTEMAS') OR (frmprincipal.AreaUsuario = 'GERENCIA DE ADMINISTRACION')) then
    begin
    Application.MessageBox('Usted no tiene derechos para borrar o cancelar pedidos', 'Atencion', MB_ICONEXCLAMATION);
    exit;
    end;
...
procedure TfrmpedidosAgregar.FormActivate(Sender: TObject);
begin
self.WindowState:=wsMaximized;
end;

procedure TfrmpedidosAgregar.cbonom_TOportunidadChange(Sender: TObject);
begin
if cbonom_toportunidad.Text = '' then
 begin
  edTipoOportunidad.Text := '';
...
procedure TfrmpedidosAgregar.sqlpedidosBeforePost(DataSet: TDataSet);
begin
...
procedure TfrmpedidosAgregar.cbivaExit(Sender: TObject);
begin
if btnalta.Caption='&Guardar' then
   begin
   CalculaTotales(SqlRejillaAlta);
...
procedure TfrmpedidosAgregar.cbonom_TOportunidadExit(Sender: TObject);
//var   tecla:char;
begin
    querys('select idUnidadNegocio,Descripcion from UnidadNegocio where nombre='+quotedstr(cbonom_TOportunidad.Text),0,dmreportes.sqlgeneral);
   edTipoOportunidad.Text:=dmreportes.sqlgeneral.fieldbyname('idUnidadNegocio').AsString;
...
procedure TfrmpedidosAgregar.btnImpPedClick(Sender: TObject);
begin
...
procedure TfrmpedidosAgregar.CargaUnidadNegocio;
begin
