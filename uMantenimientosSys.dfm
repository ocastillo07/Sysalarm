object frmMantenimientosSys: TfrmMantenimientosSys
  Left = 259
  Top = 118
  Width = 525
  Height = 210
  Caption = 'Pasar Clientes Cancelados a Base de Datos de Exclientes'
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object RzLabel1: TRzLabel
    Left = 184
    Top = 16
    Width = 309
    Height = 78
    Caption = 
      'Condiciones de Traspaso:'#13#10'** Cuentas con Contrato (CAN) Cancelad' +
      'o'#13#10'** Cuentas sin abogado'#13#10'** Cuentas con Fecha de Cancelacion m' +
      'ayor a un a'#241'o'#13#10'** Cuentas con Fecha de Ultima Se'#241'al mayor a un a' +
      #241'o'#13#10'** Cuentas con pagos mayores a un a'#241'o'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    BlinkIntervalOff = 5000
    BlinkIntervalOn = 20
  end
  object rglimite: TRzRadioGroup
    Left = 16
    Top = 16
    Width = 161
    Height = 65
    Caption = 'Limite para Trasapaso'
    Color = clWhite
    ItemHotTrack = True
    ItemIndex = 0
    Items.Strings = (
      'Si, usar limite'
      'No, no usar limite')
    TabOrder = 1
    OnClick = rglimiteClick
  end
  object btnpasar: TRzBitBtn
    Left = 336
    Top = 112
    Width = 161
    Height = 57
    Caption = 'Pasar a NO Clientes'
    HotTrack = True
    TabOrder = 0
    OnClick = btnpasarClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object edlimite: TRzEdit
    Left = 56
    Top = 88
    Width = 121
    Height = 21
    Alignment = taRightJustify
    Color = clWhite
    DisabledColor = clWhite
    FrameHotTrack = True
    FrameHotStyle = fsGroove
    FrameVisible = True
    TabOrder = 2
  end
  object sqlgeneral: TMyQuery
    Connection = DMAccesos.conexion
    SQL.Strings = (
      'select * from notas where idnota = 6')
    FetchAll = True
    Left = 24
    Top = 120
  end
  object sqlgeneral2: TMyQuery
    Connection = DMAccesos.conexion
    SQL.Strings = (
      'explain select id from notas')
    FetchAll = True
    Left = 64
    Top = 120
  end
  object sqlgeneral3: TMyQuery
    Connection = DMAccesos.conexion
    FetchAll = True
    Left = 104
    Top = 120
  end
  object sqlgeneral1: TMyQuery
    Connection = DMAccesos.conexion
    FetchAll = True
    Left = 136
    Top = 120
  end
end
